...
private struct PreparedLegacyWorld {
    ...
    let advancements: ElysiumAdvancementStorageRow?
    let chunks: [ElysiumChunkStorageRow]
    let residentReservations: [LegacyResidentReservation]

    var primitiveImport: ElysiumLegacyWorldImport? {
        try? ElysiumLegacyWorldImport(world: world, player: player,
...
enum LegacySaveMigration {
    ...
    private static func cleanupMappedError(coordinator: ElysiumStorageCoordinator,
...
#if DEBUG
    static func _testCleanupMappedError(coordinator: ElysiumStorageCoordinator,
                                        primary: SaveDBOpenError) -> SaveDBOpenError {
    ...
    static func run(databaseURL: URL, coordinator: ElysiumStorageCoordinator,
                    storage: ElysiumLegacyCoreStorage,
    ...
    private static func importSource(databaseURL: URL,
                                     coordinator: ElysiumStorageCoordinator,
                                     storage: ElysiumLegacyCoreStorage,
        ...
        for filename in validWorldNames {
            ...
            do {
                _ = try withElysiumLockRank(.saveDB) { try storage.importLegacyWorld(value) }
            } catch {
        ...
        do {
            ...
            try withElysiumLockRank(.saveDB) { try storage.prepareLegacyMigrationRename() }
            try validateLease(preflight, namespaceName: sourceName, stage: .migrationBarrier)
        } catch {
        ...
        let reopenedStorage: ElysiumLegacyCoreStorage
        var reopenedCoordinator: ElysiumStorageCoordinator?
        do {
#if DEBUG
            ...
            reopenedCoordinator = try withElysiumLockRank(.saveDB) {
                try ElysiumStorageCoordinator.open(databaseURL: databaseURL)
            }
            reopenedStorage = try withElysiumLockRank(.saveDB) {
        ...
        do {
            ...
            try createDurableMarker(parentFD: preflight.parentFD,
                                    ...
                                    stage: .migrationDirectorySync)
            try validateLease(preflight, namespaceName: backupName, stage: .migrationDirectorySync)
            print("[saves] migrated \(importedWorldCount) worlds, \(importedChunkCount) chunks into elysium.db (old files kept in saves-legacy-backup)")
            fflush(stdout)
    ...
    private static func prepareWorld(filename: String, manifest: LegacyManifest,
        ...
        do {
            worldRow = try ElysiumWorldStorageRow(id: record.id, json: worldJSON,
        ...
        let player = try optionalJSONRow(
            ...
            ledger: ledger, reservations: &reservations) { json in
                try ElysiumPlayerJSONStorageRow(world: record.id, json: json)
            }
        let advancements = try optionalJSONRow(
            ...
            ledger: ledger, reservations: &reservations) { json in
                try ElysiumAdvancementStorageRow(world: record.id, json: json)
            }
        ...
        var keys = Set<ElysiumChunkStorageKey>()
        var chunks: [ElysiumChunkStorageRow] = []
        ...
        for candidate in candidates.sorted(by: { unsignedUTF8Less($0.components.last!, $1.components.last!) }) {
            ...
            let key: ElysiumChunkStorageKey
            let row: ElysiumChunkStorageRow
            do {
                key = try ElysiumChunkStorageKey(world: record.id,
                                                dimension: tuple.dimension,
                                                chunkX: tuple.chunkX, chunkZ: tuple.chunkZ)
                guard keys.insert(key).inserted else {
                    throw SaveDBOpenError(stage: .migrationManifest, result: .invalidSource)
                }
                row = try ElysiumChunkStorageRow(key: key, data: bytes)
    ...
    private static func proveEquivalence(manifest: LegacyManifest, rootFD: Int32,
        ...
        for filename in worldNames {
            ...
            let player: ElysiumPlayerJSONStorageRow?
            let advancements: ElysiumAdvancementStorageRow?
            let keys: [ElysiumChunkStorageKey]
            do {
                if let preflight {
                    try validateLease(preflight, namespaceName: namespaceName,
                                      stage: .migrationImport)
                }
                (world, player, advancements, keys) = try withElysiumLockRank(.saveDB) {
            ...
            for chunk in prepared.chunks.sorted(by: { chunkKeyLess($0.key, $1.key) }) {
                ...
                let stored: Data?
                do {
                    stored = try withElysiumLockRank(.saveDB) {
    ...
    private static func verifyDatabaseBinding(coordinator: ElysiumStorageCoordinator,
                                              preflight: LegacyMigrationPreflight) throws
        -> LegacyFileIdentity {
        ...
        do {
            try withElysiumLockRank(.saveDB) {
    ...
    private static func chunkKeyLess(_ lhs: ElysiumChunkStorageKey,
