...
// =============================================================================
// Load-path profiler (ELYSIUM_PROF=1) — aggregates per-stage wall time
// =============================================================================
public final class LoadProf {
    public static let shared = LoadProf()
    public let enabled = ProcessInfo.processInfo.environment["ELYSIUM_PROF"] != nil
...
public final class GameCore {
    ...
    private let saveQueue = DispatchQueue(label: "elysium.save", qos: .utility)
    private let rpgLocalPreferenceQueue = DispatchQueue(
        label: "elysium.rpg-local-preferences", qos: .utility)
    private let rpgPlayerPersistenceQueue = DispatchQueue(
        label: "elysium.rpg-player-persistence", qos: .utility)
    ...
    private let rpgLocalPreferenceQueueSpecificKey = DispatchSpecificKey<UInt8>()
    ...
    init(db: SaveDB, localSettingsStore: LocalSettingsStore) {
        ...
        rpgLocalPreferenceQueue.setSpecific(
            key: rpgLocalPreferenceQueueSpecificKey,
            value: rpgLocalPreferenceQueueSpecificValue)
    ...
    package func commitSignEdit(token: SignCommitToken,
                                lines: ElysiumFourSignLines) -> Bool {
    ...
    @MainActor
    public func savedWorldMaintenanceAllowsTransitions() -> Bool {
        !savedWorldMaintenance.isHeld
    }

    @MainActor
    public func acquireSavedWorldMaintenance() -> SavedWorldMaintenanceCoordinator.Token? {
        savedWorldMaintenance.acquire()
    }

    @MainActor
    public func revalidateSavedWorldMaintenance(
        _ token: SavedWorldMaintenanceCoordinator.Token
    ) -> Bool {
        savedWorldMaintenance.revalidate(token)
    }
    ...
    ) -> SavedWorldDeleteOperation? {
        guard !inWorld, token.value > lastConsumedSavedWorldDeleteToken,
              savedWorldDeleteRequestHasBoundedCanonicalShape(request) else { return nil }
        let requestIdentity = savedWorldDeleteRequestIdentity(request)
        guard requestIdentity.count == 32,
              let admission = savedWorldMaintenance.admitDelete(
                token, requestIdentity: requestIdentity,
                screenIdentity: screenIdentity,
                launchContextIdentity: launchContextIdentity),
              savedWorldConstantTimeEqual32(
                admission.requestIdentity, requestIdentity) else { return nil }
        lastConsumedSavedWorldDeleteToken = token.value
        drainAcceptedRPGPersistenceWriters()
        return SavedWorldDeleteOperation(db: db, request: request, admission: admission)
    }

    @MainActor
    public func releaseSavedWorldMaintenance(
        _ token: SavedWorldMaintenanceCoordinator.Token
    ) {
        savedWorldMaintenance.release(token)
    }

    @MainActor
    public func finishSavedWorldDeleteOperation(
        _ operation: SavedWorldDeleteOperation,
        outcome: SavedWorldDeleteOutcome,
        screenIdentity: UInt64,
        launchContextIdentity: UUID
    ) -> Bool {
        guard savedWorldMaintenance.revalidate(operation.token),
              operation.screenIdentity == screenIdentity,
              operation.launchContextIdentity == launchContextIdentity,
              savedWorldConstantTimeEqual32(
                operation.requestIdentity,
                savedWorldDeleteRequestIdentity(operation.request)),
              operation.claimCompletion(outcome) else { return false }
        switch outcome {
        case .direct, .recovered:
            for worldID in operation.selectedWorldIDs {
                retireRPGCommittedPlayerOmission(
    ...
    public func deleteWorld(_ id: String) {
        guard savedWorldMaintenanceAllowsTransitions(),
              !inWorld || worldRec?.id != id else { return }
        drainAcceptedRPGPersistenceWriters()
        db.deleteWorld(id)
    ...
    /// Establishes the deletion cut after every already-accepted RPG local
    /// preference and player-row writer. New writers are excluded by the shared
    /// saved-world maintenance admission held by the main actor.
    private func drainAcceptedRPGPersistenceWriters() {
        if DispatchQueue.getSpecific(key: rpgLocalPreferenceQueueSpecificKey)
            != rpgLocalPreferenceQueueSpecificValue {
            rpgLocalPreferenceQueue.sync {}
        }
        withRPGPlayerPersistenceQueueSync {}
    }
...
#if DEBUG
    func _testInstallSavedWorldOmission(worldID: String) {
        let digest = try! LANV6SHA256Digest(bytes: [UInt8](repeating: 0xA5, count: 32))
        withRPGPlayerPersistenceQueueSync {
            rpgCommittedPlayerOmissions[worldID] = RPGLegacyPlayerOmissionIdentity(
                worldID: worldID, worldEntryGeneration: 1,
                envelopeVersion: 1, sourceDigest: digest)
        }
    }

    func _testHasSavedWorldOmission(worldID: String) -> Bool {
        withRPGPlayerPersistenceQueueSync {
            rpgCommittedPlayerOmissions[worldID] != nil
        }
    }
#endif
    ...
    private func withSaveQueueSync<T>(_ body: () throws -> T) rethrows -> T {
        if DispatchQueue.getSpecific(key: saveQueueSpecificKey) == saveQueueSpecificValue {
            preconditionElysiumLockRank(.saveQueue)
            return try body()
        }
        return try saveQueue.sync {
            try withElysiumLockRank(.saveQueue, body)
        }
    }
    ...
    private func withSaveQueueAsync(_ body: @escaping () -> Void) {
        saveQueue.async {
            withElysiumLockRank(.saveQueue, body)
        }
    }
    ...
    func _testSaveQueueSelfEntry() -> Int {
        withSaveQueueSync {
            withSaveQueueSync { elysiumCurrentLockRank() }
        }
    }
#endif
    ...
    public func keyDown(binding action: ElysiumGameBindingAction, configuredCode: String,
    ...
    public func keyUp(binding action: ElysiumGameBindingAction, configuredCode: String) {
