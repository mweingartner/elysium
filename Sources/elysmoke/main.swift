// Headless smoke tests for ElysiumCore. The frozen golden baselines
...
registerAllItems()
// 1186 baseline items + appended Elysium items at the END so every baseline id
// is unchanged.
...
if let g = loadJSON("biome-goldens.json") {
    ...
    // native baseline since the vanilla snow-lapse fix (ELYSIUM_REGOLD regenerates)
...
if let g = loadJSON("terrain-goldens.json") {
    ...
    // ELYSIUM_REGOLD=1 after deliberate generation changes)
    let tRegold = ProcessInfo.processInfo.environment["ELYSIUM_REGOLD"] != nil
...
if let g = loadJSON("feature-goldens.json") {
    ...
    // native baseline since the #26 worldgen quality pass (regenerate with
    // ELYSIUM_REGOLD=1 after deliberate generation changes)
    let fRegold = ProcessInfo.processInfo.environment["ELYSIUM_REGOLD"] != nil
...
if let g = loadJSON("mesh-goldens.json") {
    ...
    if let lights = g["lights"] as? [[String: Any]] {
        ...
        let lRegold = ProcessInfo.processInfo.environment["ELYSIUM_REGOLD"] != nil
        for l in lights {
    ...
    // (regenerate with ELYSIUM_REGOLD=1 after deliberate mesher changes)
    let meshRegold = ProcessInfo.processInfo.environment["ELYSIUM_REGOLD"] != nil
...
if let g = loadJSON("worldsim-goldens.json") {
    ...
    // native baseline since the #26 worldgen quality pass (regenerate with
    // ELYSIUM_REGOLD=1 after deliberate generation changes)
    let wsRegold = ProcessInfo.processInfo.environment["ELYSIUM_REGOLD"] != nil
...
if let g = loadJSON("entity-goldens.json") {
    ...
    // ELYSIUM_REGOLD=1 after deliberate entity-behavior changes)
    let zooGold = g["zooStages"] as! [[String: Any]]
    let zooRegold = ProcessInfo.processInfo.environment["ELYSIUM_REGOLD"] != nil
    ...
    // (regenerate with ELYSIUM_REGOLD=1 after deliberate physics changes)
    let combatGold = g["combatStages"] as! [[String: Any]]
    let combatRegold = ProcessInfo.processInfo.environment["ELYSIUM_REGOLD"] != nil
    ...
    // regression baseline now (regenerate with ELYSIUM_REGOLD=1 after deliberate
    // physics changes) — these are native baselines, regenerated deliberately
    let physGold = g["physStages"] as! [[String: Any]]
    let regold = ProcessInfo.processInfo.environment["ELYSIUM_REGOLD"] != nil
    ...
    // with ELYSIUM_REGOLD=1 after deliberate spawning changes)
    if ProcessInfo.processInfo.environment["ELYSIUM_REGOLD"] != nil {
        for path in goldenPaths("entity-goldens.json") {
            guard let d = FileManager.default.contents(atPath: path),
                  var obj = (try? JSONSerialization.jsonObject(with: d)) as? [String: Any] else { continue }
...
// terrain-dependent systems goldens re-baseline under ELYSIUM_REGOLD
let sysRegold = ProcessInfo.processInfo.environment["ELYSIUM_REGOLD"] != nil
