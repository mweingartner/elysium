...
final class AppInputRouter {
    ...
    private var configuredBindingPresses = PebbleConfiguredBindingPressLedger()
    private var modifierSynthesizer = PebbleModifierEdgeSynthesizer()
    ...
    private func routeUnmappedScreenText(_ event: NSEvent) -> Bool {
        ...
        let result = PebbleTextEventIngressAdapter.route(
    ...
    func flagsChanged(with event: NSEvent) {
        ...
        if !modifiers.contains(.shift),
           let terminal = PebbleTerminalKey(rawValue: "ShiftLeft") {
            release(terminal: terminal, fallbackCode: "ShiftLeft", game: game)
        }
        if !modifiers.contains(.control),
           let terminal = PebbleTerminalKey(rawValue: "ControlLeft") {
    ...
    func release(event: NSEvent) {
        guard exhaustionLatch.mayDispatchRelease(),
              ...
              let terminal = PebbleTerminalKey(rawValue: code) else { return }
        release(terminal: terminal, fallbackCode: code, game: game)
    }

    private func release(terminal: PebbleTerminalKey, fallbackCode: String, game: GameCore) {
    ...
    private func makePhysicalEvent(_ event: NSEvent)
        -> (event: PebbleKeyEvent, fingerprint: AppKeyEventFingerprint, nowMilliseconds: UInt64)? {
        guard !routingSerialExhausted,
              let terminalName = KEYCODE_MAP[event.keyCode],
              let terminal = PebbleTerminalKey(rawValue: terminalName) else { return nil }
        ...
        let modifiers = Self.modifiers(from: event.modifierFlags)
        let keyEvent = PebbleKeyEvent(terminal: terminal, modifiers: modifiers,
    ...
    private func route(_ event: PebbleKeyEvent, fingerprint: AppKeyEventFingerprint,
        ...
        let contexts: Set<PebbleBindingContext> = game.hasWorld()
    ...
    private func routeToScreen(_ event: PebbleKeyEvent, appKitEvent: NSEvent?,
        ...
        let typed = PebbleTextEventIngressAdapter.route(
    ...
    private func eligibleObjectTemplateCommand(_ event: PebbleKeyEvent, game: GameCore,
    ...
    private func independentCommand(_ event: PebbleKeyEvent, hasWorld: Bool) -> ResolvedKeyCommand? {
    ...
    private func dispatch(_ command: ResolvedKeyCommand, event: PebbleKeyEvent,
    ...
    private static func modifiers(from flags: NSEvent.ModifierFlags) -> PebbleKeyModifiers {
        var value: PebbleKeyModifiers = []
