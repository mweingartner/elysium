...
struct LANHostLaunchRequest {
    ...
    init(joinCode: String?, port: UInt16?, identity: UUID = UUID()) {
...
final class LANLobbyScreen: Screen {
    ...
    override func initScreen(_ ui: UIManager, _ game: GameCore) {
        ...
        let hostButton = Button(left + 10, y, 112, 18, "Host World", { [weak self, weak ui, weak game] in
            ...
            if !game.hasWorld() {
                elysiumMainActorSync {
                    ui.open(WorldSelectScreen(lanHostRequest: LANHostLaunchRequest(
                        joinCode: code, port: port)), game)
                }
                return
            }
            do {
                try elysiumMainActorSync {
                    try self.manager.startHost(
                        game: game, requestedJoinCode: code, requestedPort: port)
                }
                if let code {
        ...
        buttons.append(Button(left + 130, y, 92, 18, "Browse LAN", {
            [weak self, weak game] in
            guard let self, let game else { return }
            do { try elysiumMainActorSync { try self.manager.startBrowsing(game: game) } }
            catch let error as LANTransportError { pushChat("§c" + error.description) }
            catch { pushChat("§cLAN browse failed.") }
        }))
        ...
        buttons.append(Button(left + 10, y, panelW - 20, 18, "Join World", { [weak self, weak game] in
            guard let self, let game else { return }
            elysiumMainActorSync { self.joinWorld(game) }
        }))
    ...
    override func onKey(_ ui: UIManager, _ game: GameCore, _ key: String) -> Bool {
        if key == "Enter" {
            if joinHostField.focused || joinPortField.focused || joinCodeField.focused || playerNameField.focused {
                elysiumMainActorSync { joinWorld(game) }
                return true
            }
        }
