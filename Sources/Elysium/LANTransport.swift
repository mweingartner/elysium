...
final class LANMultiplayerManager {
    ...
    private let queue = DispatchQueue(label: "com.briangao.elysium.lan")
    ...
    private func configureClientReplicationHooks(for game: GameCore) {
        ...
        game.lanBlockIntentHandler = { [weak self] intent in
            ...
            if ProcessInfo.processInfo.environment["ELYSIUM_LAN_PROBE_LOG"] != nil {
                elysiumLANProbeLog("client_send_block_intent action=\(intent.action.rawValue) target=\(intent.x),\(intent.y),\(intent.z) cell=\(intent.cell)")
            }
            self.queue.async { [weak self] in
                guard let self, let peer = self.clientPeer else { return }
        ...
        game.lanRPGIntentHandler = { [weak self] intent in
            guard let self else { return }
            self.queue.async { [weak self] in
                guard let self, let peer = self.clientPeer else { return }
                self.send(.rpgIntent(playerID: self.localPeerID, intent: intent), to: peer)
            }
        }
    ...
    func startHost(game: GameCore, requestedJoinCode: String?, requestedPort: UInt16?) throws {
        ...
        hostWorldSummary = LANWorldSummary(
            ...
            worldName: rec?.name ?? "Elysium World",
        ...
        joinCode = code
        hostPort = port
        hostedWorldName = hostWorldSummary?.worldName ?? "Elysium World"
        activeGame = game
    ...
    @MainActor
    func startBrowsing(game: GameCore) throws {
        guard game.savedWorldMaintenanceAllowsTransitions() else {
            throw LANTransportError.alreadyBusy
        }
        browser?.cancel()
        ...
        newBrowser.browseResultsChangedHandler = { [weak self] results, _ in
            ...
            DispatchQueue.main.async {
                ...
                if hosts.isEmpty {
                    self?.appendStatus("No Elysium LAN worlds discovered yet.")
                } else {
                    self?.appendStatus("Discovered \(hosts.count) Elysium LAN world\(hosts.count == 1 ? "" : "s").")
                }
            }
        }
    ...
    func connectToDiscovered(
        _ host: LANDiscoveredHost, game: GameCore,
        playerName: String, joinCode rawJoinCode: String
    ) throws {
        guard game.savedWorldMaintenanceAllowsTransitions() else {
            throw LANTransportError.alreadyBusy
        }
    ...
    @MainActor
    func directConnect(
        host rawHost: String, port rawPort: String,
        joinCode rawJoinCode: String, playerName: String, game: GameCore
    ) throws {
        guard game.savedWorldMaintenanceAllowsTransitions() else {
            throw LANTransportError.alreadyBusy
        }
    ...
    func tickReplication(game: GameCore) {
        ...
        if transportState.isHosting {
            ...
            let state = makeLANPlayerState(player, playerID: localPeerID, displayName: NSFullUserName(), dimension: game.dim.rawValue, includeRPG: false)
            queue.async { [weak self] in
                guard let self, let peer = self.clientPeer else { return }
    ...
    private func handleHostMessage(_ message: LANMultiplayerMessage, from peer: LANWirePeer,
        ...
        case .clientHello(let playerID, let playerName, let rawJoinCode, let elysiumVersion):
            ...
            guard elysiumVersion == ELYSIUM_VERSION else {
                send(.serverReject(reason: "Elysium version mismatch: host is \(ELYSIUM_VERSION)."), to: peer)
                peer.connection.cancel()
                appendStatus("Rejected LAN join from \(name): version mismatch (\(elysiumVersion)).")
                return
            }
    ...
    private func reportBlockIntentResult(_ result: LANBlockIntentResult, playerID: String, peerName: String, tick: Int) {
        if ProcessInfo.processInfo.environment["ELYSIUM_LAN_PROBE_LOG"] != nil {
            ...
            case .applied(let changes):
                elysiumLANProbeLog("host_block_intent_result player=\(peerName) applied changes=\(changes.count)")
            case .ignored(let reason):
                elysiumLANProbeLog("host_block_intent_result player=\(peerName) ignored reason=\(reason)")
            case .rejected(let reason):
                elysiumLANProbeLog("host_block_intent_result player=\(peerName) rejected reason=\(reason)")
            }
        }
        switch result {
        case .applied(let changes):
        ...
        guard let game = activeGame, game.hasWorld() else { return }
            ...
            case .createCharacter:
                guard !state.created else {
                    reject("character already created")
                    return
                }
                guard let draft = intent.draft else {
                    reject("missing character draft")
                    return
                }
            ...
            case .learnSkill:
                ...
                if let error = rpgLearnSkill(skillID, in: &state) {
                    reject(error.description)
                    return
                }
                publish(state)
            case .prepareSkill:
                ...
                if let error = rpgPrepareSkill(skillID, in: &state) {
                    reject(error.description)
                    return
                }
                publish(state)
            case .unprepareSkill:
                ...
                if let error = rpgUnprepareSkill(skillID, in: &state) {
                    reject(error.description)
                    return
                }
                publish(state)
            ...
            case .prepareSpell:
                ...
                if let error = rpgPrepareSpell(spellID, in: &state) {
                    reject(error.description)
                    return
                }
                publish(state)
...
func lanPeerRecordJSON(_ record: LANPeerRecordSnapshot) -> [String: Any] {
    ...
    if let rpg = record.rpg, let rpgJSON = lanJSONObject(rpg) {
        out["rpg"] = rpgJSON
    }
...
func lanPeerRecordSnapshot(fromStoredJSON data: [String: Any], playerID: String) -> LANPeerRecordSnapshot? {
    ...
    let rpg = lanDecode(RPGCharacterState.self, from: data["rpg"]) ?? state.rpg
...
func runLANCommand(_ game: GameCore, _ args: [String]) {
    ...
    do {
        ...
        case "browse", "discover":
            try manager.startBrowsing(game: game)
            ok("Browsing for Elysium LAN worlds.")
        case "hosts", "list":
            let hosts = manager.discoveredHosts
            if hosts.isEmpty {
                ok("No Elysium LAN worlds discovered. Run /lan browse first.")
            } else {
        ...
        case "join":
            ...
            if let index = Int(args[1]), index >= 0, index < manager.discoveredHosts.count {
                try manager.connectToDiscovered(
                    manager.discoveredHosts[index], game: game,
                    playerName: playerName(from: 3), joinCode: args[2])
                ...
                try manager.directConnect(
                    host: args[1], port: args[2], joinCode: args[3],
                    playerName: playerName(from: 4), game: game)
