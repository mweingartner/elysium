...
/// App-layer identity for one script-originated Ollama request. `ScriptRuntime` request ids restart
/// at one for every world session, so the runtime id alone is not globally unique. Keeping the
/// opaque session id in this key prevents a cancelled old world's deferred URLSession completion
/// from deleting the tracking entry for a new world's coincidentally identical request id.
struct OllamaScriptRequestKey: Hashable, Sendable {
    let sessionID: UUID
...
final class OllamaAgentService {
    ...
    private struct ScriptBrokerTask {
        /// A second identity protects against accidental replacement with the exact same key. The
        /// normal broker never reuses a key, but cleanup should still remove only its own task.
        let identity: UUID
        let task: Task<Void, Never>
    }

    /// `Task` and `DispatchQueue` both require sendable captures. This narrow relay is unchecked
    /// only because it is created on the service's owning thread and dereferenced exclusively by
    /// the main-queue closure below; it does not make the mutable service itself generally sendable.
    private final class ScriptBrokerCompletionTarget: @unchecked Sendable {
        weak var service: OllamaAgentService?
        let completion: (String?, String?) -> Void

        init(service: OllamaAgentService, completion: @escaping (String?, String?) -> Void) {
            self.service = service
            self.completion = completion
        }
    }

    /// The decoded script-visible reply is capped separately below. This transport ceiling applies
    /// while bytes are arriving, before JSON decoding, so a hostile or malfunctioning loopback
    /// listener cannot make one of the two permitted broker requests buffer an unbounded body.
    static let scriptReplyTransportByteLimit = 65_536
    /// The decoded text must fit through the Lua `ScriptValue` string boundary used by both
    /// `ai.await` resume arguments and `ai.replied` payloads. Characters are not bytes: one model
    /// response containing emoji could otherwise pass a character count and fail at delivery.
    static let scriptReplyUTF8ByteLimit = ScriptingStorageCaps.defaults.value.stringBytes

    /// design.md §9.6: in-flight script broker requests (`ai.ask`/`ai.await`), keyed by both
    /// world-session identity and the session-local `ScriptRuntime` request id. This supports
    /// precise cancellation without letting an old world's late cleanup collide with a new world.
    private var scriptBrokerTasks: [OllamaScriptRequestKey: ScriptBrokerTask] = [:]
    ...
    /// cancellable, never blocks the caller. `requestKey.requestID` is `ScriptRuntime`'s own
    /// outbox id and `requestKey.sessionID` is generated by `AIScriptBroker`; neither is sent over
    /// the network.
    func generateScriptReply(
        requestKey: OllamaScriptRequestKey, model: String, prompt: String, maxChars: Int,
        ...
        let taskIdentity = UUID()
        let session = self.session
        let completionTarget = ScriptBrokerCompletionTarget(service: self, completion: completion)
        let task = Task {
            ...
            do {
                let (data, response) = try await boundedOllamaData(
                    session: session,
                    request: request,
                    maximumBytes: Self.scriptReplyTransportByteLimit
                )
                try Task.checkCancellation()
                guard let http = response as? HTTPURLResponse,
                      ...
                      let reply = decoded.message?.content,
                      !reply.isEmpty else {
                    throw URLError(.badServerResponse)
                }
                outcome = (Self.scriptVisibleReply(reply), nil)
            } catch {
                let nsError = error as NSError
                let cancelled = Task.isCancelled
                    || error is CancellationError
                    || (nsError.domain == NSURLErrorDomain && nsError.code == NSURLErrorCancelled)
                outcome = (nil, cancelled ? "cancelled" : "transport")
            }
            DispatchQueue.main.async {
                if completionTarget.service?.finishScriptRequest(
                    requestKey, identity: taskIdentity
                ) == true {
                    completionTarget.completion(outcome.text, outcome.error)
                } else {
                    completionTarget.completion(nil, "cancelled")
                }
            }
        }
        if let displaced = scriptBrokerTasks.updateValue(
            ScriptBrokerTask(identity: taskIdentity, task: task), forKey: requestKey
        ) {
            // Reuse is a caller bug, but fail closed: do not leave the displaced network request
            // running and do not let its later completion erase the replacement.
            displaced.task.cancel()
        }
    }

    /// Truncates on an extended-grapheme boundary and never returns more than the Lua marshaler's
    /// UTF-8 byte cap. The transport body has already been streaming-limited to 64 KiB, so this
    /// bounded character walk cannot become unbounded post-decode work.
    static func scriptVisibleReply(_ reply: String) -> String {
        let limit = max(0, scriptReplyUTF8ByteLimit)
        guard reply.utf8.count > limit else { return reply }
        var result = ""
        result.reserveCapacity(min(limit, reply.utf8.count))
        var byteCount = 0
    ...
    @discardableResult
    private func finishScriptRequest(_ key: OllamaScriptRequestKey, identity: UUID) -> Bool {
        guard scriptBrokerTasks[key]?.identity == identity else { return false }
        scriptBrokerTasks.removeValue(forKey: key)
        return true
    }

    /// Cancels exactly one script request. The caller supplies the session-qualified key, so an
    /// old runtime cannot cancel a new world's request that reused the same numeric request id.
    @discardableResult
    func cancelScriptRequest(_ key: OllamaScriptRequestKey) -> Bool {
        guard let tracked = scriptBrokerTasks.removeValue(forKey: key) else { return false }
        tracked.task.cancel()
        return true
    }

    /// Internal diagnostics used by focused app-layer tests and cancellation plumbing.
    func hasScriptRequest(_ key: OllamaScriptRequestKey) -> Bool {
        scriptBrokerTasks[key] != nil
    }

    var scriptBrokerRequestCount: Int { scriptBrokerTasks.count }
    ...
    func cancelAllScriptRequests() {
        let tracked = Array(scriptBrokerTasks.values)
        scriptBrokerTasks.removeAll()
        for entry in tracked { entry.task.cancel() }
    }
