...
package enum PebbleTextInsertResult: Equatable, Sendable {
    case accepted(characterCount: Int, utf8ByteCount: Int)
    case rejectedInvalid
    case rejectedLimit
}

package let pebbleTextEventMaximumUTF8Bytes = 65_536
...
package func pebbleByteSequenceFitsTextEventEnvelope<Bytes: Sequence>(
    _ bytes: Bytes, maximumBytes: Int = pebbleTextEventMaximumUTF8Bytes
...
package func pebbleTextFitsEventEnvelope(
    _ text: String, maximumBytes: Int = pebbleTextEventMaximumUTF8Bytes
) -> Bool {
    pebbleByteSequenceFitsTextEventEnvelope(text.utf8, maximumBytes: maximumBytes)
}

package enum PebbleTextEventIngressResult: Equatable, Sendable {
    case blocked
...
package enum PebbleTextPasteIngressAdapter {
...
package func pebbleValidateTextCharacter(_ value: Character) -> PebbleTextValidationResult {
    ...
    return .accepted(PebbleValidatedCharacter(value: value, scalarCount: scalarCount,
...
package struct PebbleBoundedTextBuffer: Equatable, Sendable {
    ...
    package init(_ text: String = "", limit: PebbleTextLimit) {
        self.text = ""
        characterCount = 0
        utf8ByteCount = 0
    ...
    package mutating func replaceAtomically(_ proposal: String, limit: PebbleTextLimit) -> Bool {
    ...
    package mutating func insertWholeProposalAtomically(
        _ proposal: String, atCharacterOffset offset: Int, limit: PebbleTextLimit
    ) -> PebbleTextInsertResult {
        ...
        for character in proposal {
            guard case .accepted(let accepted) = pebbleValidateTextCharacter(character) else {
    ...
    private static func validatedCounts(_ proposal: String, limit: PebbleTextLimit)
...
package enum PebbleTextFocusBeginResult: Equatable, Sendable {
    case started
...
package enum PebbleTextActivationCause: CaseIterable, Hashable, Sendable {
    case primaryClick
    case initialOpen
...
package struct PebbleTextIngressStateMachine: Equatable, Sendable {
    private enum Phase: Equatable, Sendable {
        case idle
        case focusing(PebbleTextOwnerToken, notificationDelivered: Bool)
    }
    private var phase: Phase = .idle
    package private(set) var readiness: PebbleTextOwnerToken?

    package init() {}

    package mutating func begin(_ token: PebbleTextOwnerToken,
                                ownerAndResponderAlreadyReady: Bool) -> PebbleTextFocusBeginResult {
        switch phase {
        case .idle:
    ...
    package mutating func markNotificationDelivered(for token: PebbleTextOwnerToken) -> Bool {
    ...
    package mutating func complete(_ token: PebbleTextOwnerToken,
    ...
    package mutating func cancel(_ token: PebbleTextOwnerToken? = nil) {
        if let token, case .focusing(let current, _) = phase, current != token { return }
        readiness = nil
        phase = .idle
    }

    package func isReady(_ token: PebbleTextOwnerToken) -> Bool {
...
package enum PebbleTextFocusTransactionResult: Equatable, Sendable {
    case committed
    case idempotentReady
    case coalesced
    case rejected
...
package final class PebbleTextFocusTransactionAdapter {
    private var machine = PebbleTextIngressStateMachine()

    package init() {}

    package var ingressBlocked: Bool { machine.ingressBlocked }
    package func isReady(_ token: PebbleTextOwnerToken) -> Bool { machine.isReady(token) }
    package func cancel(_ token: PebbleTextOwnerToken? = nil) { machine.cancel(token) }

    package func perform(
        token: PebbleTextOwnerToken,
    ...
    ) -> PebbleTextFocusTransactionResult {
...
package struct PebbleTextAccessibilityIdentity: Equatable, Sendable {
    package let screenIdentity: UInt64
    package let presentationGeneration: UInt64
    package let descriptorID: String
...
package func pebbleTextAccessibilityIdentityIsCurrent(
    origin: PebbleTextAccessibilityIdentity,
    current: PebbleTextAccessibilityIdentity
) -> Bool {
    origin == current && origin.screenIdentity != 0 && origin.presentationGeneration != 0
}

package struct PebbleTextPresentation: Equatable, Sendable {
...
package func pebbleTextPresentation(text: String, caret: Int, visibleStart: Int,
                                    maximumWidth: Int, measure: (String) -> Int)
    -> PebbleTextPresentation {
    ...
    return PebbleTextPresentation(text: visible, visibleStart: start,
                                  caretOffset: boundedCaret, caretAdvance: max(0, advance))
}

package func pebbleCaretOffsetForClick(text: String, visibleStart: Int, clickAdvance: Double,
...
package func pebbleUTF16InsertionOffset(in value: String, characterOffset: Int) -> Int? {
...
package struct PebbleTextRect: Equatable, Sendable {
...
package func pebbleClampTextRect(_ value: PebbleTextRect,
                                 to bounds: PebbleTextRect) -> PebbleTextRect? {
    ...
    return PebbleTextRect(x: left, y: bottom, width: right - left, height: top - bottom)
}

package enum PebbleSignLineIndex: Int, CaseIterable, Sendable {
    case first, second, third, fourth
}

package struct PebbleSignLine: Equatable, Sendable {
    package private(set) var text: String
    package private(set) var utf8ByteCount: Int
    ...
    package static var empty: PebbleSignLine { PebbleSignLine() }
}

package struct PebbleFourSignLines: Equatable, Sendable {
    package var first: PebbleSignLine
    package var second: PebbleSignLine
    package var third: PebbleSignLine
    package var fourth: PebbleSignLine

    package init(first: PebbleSignLine = .empty, second: PebbleSignLine = .empty,
                 third: PebbleSignLine = .empty, fourth: PebbleSignLine = .empty) {
        self.first = first; self.second = second; self.third = third; self.fourth = fourth
    }
    package func line(_ index: PebbleSignLineIndex) -> PebbleSignLine {
        switch index { case .first: first; case .second: second; case .third: third; case .fourth: fourth }
    }
    package mutating func set(_ line: PebbleSignLine, at index: PebbleSignLineIndex) {
        switch index { case .first: first = line; case .second: second = line; case .third: third = line; case .fourth: fourth = line }
    }
    package var array: [String] { PebbleSignLineIndex.allCases.map { line($0).text } }
}

package struct PebbleSignRepairResult: Equatable, Sendable {
    package let lines: PebbleFourSignLines
    package let repaired: Bool
}

package func pebbleRepairSignLines(
    _ source: [String]?, makeWidthStep: () -> (Character) -> Int?
) -> PebbleSignRepairResult {
    var output = PebbleFourSignLines()
    var repaired = source == nil
    var iterator = source?.makeIterator()
    for lineIndex in PebbleSignLineIndex.allCases {
        ...
        output.set(PebbleSignLine(validatedText: text), at: lineIndex)
    }
    if iterator?.next() != nil { repaired = true }
    return PebbleSignRepairResult(lines: output, repaired: repaired)
}

package struct PebbleSignEditResult: Equatable, Sendable {
    package let line: PebbleSignLine
    package let acceptedCharacterCount: Int
    package let stoppedForCapacity: Bool
    package let stoppedForInvalidCharacter: Bool
}

package func pebbleAppendSignText(
    _ proposal: String, to line: PebbleSignLine,
    makeWidthStep: () -> (Character) -> Int?
) -> PebbleSignEditResult {
    let widthStep = makeWidthStep()
    for existing in line.text { guard widthStep(existing) != nil else {
        return PebbleSignEditResult(line: line, acceptedCharacterCount: 0,
    ...
    for character in proposal {
        guard case .accepted(let accepted) = pebbleValidateTextCharacter(character) else {
            return PebbleSignEditResult(
                line: PebbleSignLine(validatedText: result), acceptedCharacterCount: count,
    ...
    return PebbleSignEditResult(line: PebbleSignLine(validatedText: result),
                                acceptedCharacterCount: count,
                                stoppedForCapacity: false,
                                stoppedForInvalidCharacter: false)
}

package func pebbleDeleteLastSignCharacter(_ line: PebbleSignLine) -> PebbleSignLine {
    guard !line.text.isEmpty else { return line }
    return PebbleSignLine(validatedText: String(line.text.dropLast()))
}
