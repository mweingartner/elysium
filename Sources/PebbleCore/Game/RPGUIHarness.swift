...
public struct RPGUIHarnessBootstrap: Equatable {
    ...
    public static let allowedPebbleKeys: Set<String> = [
    ...
    public static func parseIfPresent(_ environment: [String: String]) -> RPGUIHarnessBootstrapResult {
        guard let selectorText = environment["PEBBLE_RPG_UI_CASE"] else { return .ordinary }
        ...
        for key in environment.keys.sorted() {
            ...
            let valueLimit = key == "PEBBLE_RPG_UI_CASE" ? 192 : 512
        ...
        let pebbleKeys = environment.keys.filter { $0.hasPrefix("PEBBLE_") }
        guard pebbleKeys.allSatisfy(allowedPebbleKeys.contains) else {
            return .rejected("RPG UI harness rejected: non-harness PEBBLE key")
        }
        guard let selector = parseSelector(selectorText) else {
            return .rejected("RPG UI harness rejected: invalid case selector")
        }
        let authorityText = environment["PEBBLE_RPG_UI_AUTHORITY"] ?? "ready"
        let viewportText = environment["PEBBLE_RPG_UI_VIEWPORT"] ?? "520x330"
        let appearanceText = environment["PEBBLE_RPG_UI_APPEARANCE"] ?? "standard"
        ...
        let summary: Bool
        switch environment["PEBBLE_RPG_UI_SEMANTIC_SUMMARY"] {
        ...
        if let raw = environment["PEBBLE_SHOT"] {
