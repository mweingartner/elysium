...
- **Frozen reference goldens** — `atlas`, `fmath`, `fmath-explog`, `items`. These are immutable reference baselines that `elysmoke`/`ELYSIUM_REGOLD` **never** regenerate. `fmath-explog` is the one category with its own generator (`scripts/fdlibm-reference/gen-explog-goldens.sh`, an independent netlib fdlibm C build): it was produced once, committed, and is never regenerated again — if your change breaks it, your change is wrong, not the golden.
- **Native baselines** — `biome`, `terrain`, `feature`, `mesh`, `worldsim`, `entity`, `systems`, `script-runtime`, `event-bus`, `scripting`. Regenerable with `ELYSIUM_REGOLD=1 swift run -c release elysmoke`, but only for *deliberate* behavior changes. `script-runtime` (the embedded Lua sandbox/determinism corpus) and `event-bus` (delivery order, coalescing, cap trips, persisted-subscription round trip) are regenerated the same way and only for a justified, reviewed change to that layer's observable behavior. `scripting` (`goldens/scripting-goldens.json`) hashes a trace of the four Appendix A scripts from `docs/scripting-and-eventing-design.md` running headlessly end to end (attribute-triggered handler/module scripts, an `ai.await` gatekeeper against a stub responder, world-wide event counting, and scripts attaching scripts) plus the kill switch, trust gate, and fault isolation — regenerate it only when `ScriptRuntime`'s observable behavior deliberately changes. `ELYSIUM_REGOLD=1` regenerates **every** native-baseline category in one pass, not just the one you're working on — after regolding, `git status`/`git diff --stat` the `goldens/` directory and revert any file you didn't mean to touch before committing (this bit a reviewer during 1b: a single elysmoke run under `ELYSIUM_REGOLD=1` silently rewrote seven unrelated categories).
...
- **Script-visible math/RNG/iteration route through the host seams, never libm.** Every Lua-visible `sin`/`cos`/`atan`/`exp`/`log`/`^`, `math.random`, and `randomseed` call is dispatched through the embedded runtime's `ScriptMath`/`ScriptRandomStream` seams (`Sources/ElysiumCore/Scripting/ScriptHostBindings.swift` wires them to `DetMath`/`RandomX`); `CLua`/`ElysiumScript` never call libm, and Swift never spells a raw `lua_`/`luaL_`/`LUA_` identifier outside `Sources/ElysiumScript/` (scanner-enforced). Table iteration order inside a script is a pure function of operation history (fixed hash seed + per-object creation-ordinal hashing), matching the sim-code no-unordered-iteration rule above. NaN sign formatting (`tostring(0/0)`, `%q` of NaN) is pinned by `goldens/script-runtime-goldens.json` as observed on arm64; cross-architecture reproduction of that one bit is a stated assumption, not yet verified elsewhere.
- **The vendored Lua interpreter is re-derived, never hand-edited.** `Sources/CLua` is Lua 5.4.8 (SHA-256-and-byte-size-pinned upstream tarball) plus the checked-in `scripts/clua/elysium.patch`. To pull a newer Lua release: point `scripts/clua/rederive.sh --tarball <path>` (or let it fetch over HTTPS) at the new tarball, update the pinned URL/SHA-256 at the top of the script, run it with `--write` to regenerate `Sources/CLua` and `scripts/clua/upstream-manifest.json` from the new sources, hand-port and re-test `scripts/clua/elysium.patch` against them (the patch is authored per release, not carried forward blindly), then run `scripts/clua/rederive.sh` without `--write` to confirm zero drift and `swift test --filter CLuaSourceTests`. Never edit a file under `Sources/CLua` directly — the drift check and the reverse-patch source test exist precisely to catch that.
- **`AttrValue` JSON is canonical, never `JSONSerialization`.** Anything that encodes or decodes an `AttrValue`/`ObjectRecord` (`Sources/ElysiumCore/Scripting/AttrValueCodec.swift`) must go through its hand-rolled recursive-descent codec: sorted UTF-8-byte-order keys, `Int64` vs `Double` kept distinct, `-0` normalized to `0`, only `true`/`false` literals. `JSONSerialization` cannot preserve that distinction and silently resolves duplicate keys — using it here would corrupt values or make output non-deterministic across a Dictionary's hash-seeded key order (same rule as "no unordered iteration" above).
- **`Entity.id` is a persisted uid, minted through the reservation, never assigned directly.** `Entity.id` is `private(set)`; a new entity gets its id from the normal mint path, which consults the durable hi/lo reservation (`installEntityIdReservation`/`raiseEntityIdReservationLimit`, `Sources/ElysiumCore/Entity/Entity.swift`) so a crash between minting and the next world-record flush can never hand out a uid an earlier session already used. A loaded save's `"id"` is adopted only when `EntityRegistry.loadEntity` decodes it as a genuine integer-typed JSON number in range — never trust a raw JSON number type coercion (a float, a string, an out-of-range value) as an id.
- **Renewing a release-surface pin is a reviewed, narrow procedure, not a rubber stamp.** `scripts/verify-elysium-storage-release-surface.sh` and `scripts/elysium-core-storage-capability-v1.json` pin exact source/object/product hashes for the checked storage surface. When a change legitimately touches `Saves.swift`, `GameCore.swift`, or the capability manifest itself: run `swift scripts/sqlite-boundary-scan.swift --root "$PWD"` and take its printed `actual=` digest for the capability-manifest entry; take the plain `shasum -a 256` of the changed source file(s) for the `EXPECTED_*_SOURCE_SHA256` pins; and, from one clean `swift build -c release` with zero warnings, compute each `EXPECTED_*_OBJECT_SHA256`/`EXPECTED_*_PRODUCT_SHA256` the same way the script itself does (`strip -S -x` a disposable copy of the artifact under `.build/out/Products/Release`, then `shasum -a 256` it) — never hand-edit a pin to an unverified value. Renew only the pins for files you actually changed; `EXPECTED_STORAGE_SOURCE_SHA256`, `EXPECTED_STORAGE_API_SHA256`, `EXPECTED_STORAGE_OBJECT_SHA256`, `EXPECTED_PLAYER_SOURCE_SHA256`, and the text-input pins must stay byte-identical unless the change genuinely touches those files. Record the old and new value of every pin you renew, and why, in the change's `build.md`.
