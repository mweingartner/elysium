...
import Security
import XCTest
@testable import PebbleReleaseGate
...
final class ReleaseGateTests: XCTestCase {
    ...
    func testOptionalDiagnosticIsDisconnectedAndSelfDisqualifying() throws {
        ...
        for relative in [
            ...
            ".githooks/pre-push", "scripts/pipeline.sh", "pebble",
    ...
    func testOptionalDiagnosticEarlyMissingToolOutputIsPrefixedCappedAndSelfDisqualifying() throws {
        ...
        environment["PATH"] = "/nonexistent/pebble-optional-diagnostic-path"
    ...
    func testOptionalDiagnosticUnexpectedRowExtractionFailureIsPrefixedCappedAndCleansUp() throws {
        ...
        try """
        #!/bin/bash
        for candidate in /tmp/pebble-release-gate-adversarial.*; do
          if [ -d "$candidate" ]; then
            printf '%s\\n' "$candidate" >> "$PEBBLE_FAKE_SWIFT_MARKER"
          fi
        done
        ...
        let preexisting = Set(try FileManager.default.contentsOfDirectory(
            atPath: "/tmp").filter { $0.hasPrefix("pebble-release-gate-adversarial.") })
        ...
        environment["PEBBLE_FAKE_SWIFT_MARKER"] = workspaceMarker.path
        process.environment = environment
        process.standardOutput = outputPipe
        process.standardError = outputPipe
    ...
    private func temporaryDirectory(_ name: String = UUID().uuidString) throws -> URL {
        let value = FileManager.default.temporaryDirectory
            .appendingPathComponent("pebble-release-gate-tests-\(name)")
    ...
    private func payload(sequence: UInt64 = 0, state: ReleaseGateState = .prepared)
        ...
        ReleaseGatePayload(
            ...
            artifacts: ReleaseArtifactIdentity(
                ...
                installedBundlePath: "/Applications/Pebble.app",
                installedExecutablePath: "/Applications/Pebble.app/Contents/MacOS/Pebble",
    ...
    func testPackageManifestBindsCapturedReleaseAndClosedFields() throws {
        ...
        let text = """
        release_path=/tmp/Pebble
        bundle_path=/tmp/repository/dist/Pebble.app
        executable_path=/tmp/repository/dist/Pebble.app/Contents/MacOS/Pebble
        pre_sign_input_sha256=\(hash)
        pre_sign_staged_sha256=\(hash)
        post_sign_executable_sha256=bbb
        bundle_id=com.briangao.pebble
        cdhash=ccc
        designated_requirement=identifier "com.briangao.pebble"
    ...
    func testAutomatedEvidenceRequiresRealPrivateLogsCountsAndClosedGateSet() throws {
        ...
        for id in AutomatedGateEvidence.requiredIDs.sorted() {
            ...
            entries.append(AutomatedGateEntry(
                ...
                passedCount: id == "pebsmoke" ? 457 : id == "appkit-text-entry" ? 2 : 1,
    ...
    func testKeychainTestIdentityCannotAliasProduction() throws {
        ...
        XCTAssertThrowsError(try KeychainReceiptIdentity.isolatedTest(
            service: "com.briangao.pebble.test.installed-signoff.x", account: "repository:x"))
        XCTAssertNoThrow(try KeychainReceiptIdentity.isolatedTest(
            service: "com.briangao.pebble.test.installed-signoff.\(UUID().uuidString)",
    ...
    func testAdversarialCategory01FixedPrepareWorkflowCleanStartAndRealBuild() throws {
        ...
        let compile = try runExecutable(URL(fileURLWithPath: "/usr/bin/xcrun"), [
            "swiftc", repositoryRoot.appendingPathComponent(
                "Tests/PebbleReleaseGateTests/Fixtures/ReleaseGateWorkflowProbe/main.swift").path,
            "-I", products.path, products.appendingPathComponent("PebbleReleaseGate.o").path,
        ...
        for scenario in ["clean", "stale-generated-release"] {
            ...
            let package = Package(name: "Fixture", products: [
              ...
              .executableTarget(name: "Pebble"), .executableTarget(name: "pebsmoke"),
              .testTarget(name: "FixtureTests", dependencies: []),
            ])
            """)
            try write("Sources/Pebble/main.swift", "print(\"fixture Pebble\")\n")
            try write("Sources/pebsmoke/main.swift", "print(\"457 passed, 0 failed\")\n")
            ...
            #!/bin/bash
            set -euo pipefail
            ! find .build -type f -name Pebble -print -quit 2>/dev/null | grep -q .
            echo '==> security: passed'
            """, executable: true)
            try write("scripts/verify-pebble-storage-release-surface.sh",
                      "#!/bin/bash\necho 'Pebble storage release surface verified'\n", executable: true)
            ...
            XCTAssertEqual(try git(["config", "user.email", "fixture@pebble.invalid"]).0, 0)
            XCTAssertEqual(try git(["config", "user.name", "Pebble Fixture"]).0, 0)
            ...
            XCTAssertEqual(evidence.entries.map(\.commandID), ["source-security", "release-build",
                "release-surface", "binary-scan", "appkit-text-entry", "xctest", "pebsmoke"])
    ...
    func testCodesignTwoChannelFixtureCorpusMatchesSwiftAndPackageParser() throws {
        let executable = "/tmp/Pebble.app/Contents/MacOS/Pebble"
    ...
    func testCodesignRawCaptureHandlesTimingFragmentationCapacityEarlyCloseAndStatus() throws {
        ...
        let expected = "/tmp/Pebble.app/Contents/MacOS/Pebble"
    ...
    func testRealPackageCodesignRequirementMatchesFreshParserAndSecurityFramework() throws {
        ...
        let release = root.appendingPathComponent("Pebble")
        ...
        let bundle = root.appendingPathComponent("dist/Pebble.app")
        try FileManager.default.createDirectory(
    ...
    func testAllSevenClosedParsersRejectWrongCountsAndForgedPassText() throws {
        let valid: [String: String] = [
            ...
            "release-surface": "Pebble storage release surface verified",
        ...
        XCTAssertThrowsError(try ClosedGateOutputParser.counts(
            commandID: "pebsmoke", output: "456 passed, 0 failed"))
        XCTAssertThrowsError(try ClosedGateOutputParser.counts(
    ...
    func testAdversarialCategory05PathAndArtifactReplacementBoundaries() throws {
        let root = try temporaryDirectory(), file = root.appendingPathComponent("Pebble")
    ...
    func testAdversarialCategory03PublicCommandSurfaceRejectsCallerForgery() throws {
        ...
        XCTAssertFalse(receipt.contains("ProcessInfo.processInfo.environment[\"PEBBLE_RELEASE_GATE"))
    ...
    func testAdversarialCategory07TemporaryGitHooksInterruptionRecoveryAndReplay() throws {
        ...
        XCTAssertEqual(try runProcess("/usr/bin/git", ["config", "user.email", "test@pebble.invalid"]).0, 0)
        XCTAssertEqual(try runProcess("/usr/bin/git", ["config", "user.name", "Pebble Test"]).0, 0)
        ...
        try executable("scripts/verify-pebble-storage-release-surface.sh",
                       "#!/bin/sh\necho 'Pebble storage release surface verified'\n")
        ...
        let package = Package(name: "Fixture", products: [
          .executable(name: "Pebble", targets: ["Pebble"]),
          .executable(name: "pebsmoke", targets: ["pebsmoke"]),
        ], targets: [.executableTarget(name: "Pebble"), .executableTarget(name: "pebsmoke"),
                     .testTarget(name: "FixtureTests")])
        """)
        try executable("Sources/Pebble/main.swift", "print(\"fixture\")\n")
        try executable("Sources/pebsmoke/main.swift", "print(\"457 passed, 0 failed\")\n")
        ...
        case "${1:-}" in
          ...
          build) mkdir -p .build/release; printf '#!/bin/sh\nexit 0\n' > .build/release/Pebble; chmod 755 .build/release/Pebble; echo 'Build complete!';;
