...
final class RPGLocalStorageStageFailureTests: XCTestCase {
    private func databaseURL(_ label: String) -> URL {
        FileManager.default.temporaryDirectory.appendingPathComponent(
            "ElysiumRPGStage-\(label)-\(UUID().uuidString).sqlite")
    }

    private func preference() throws -> ElysiumRPGLocalPreferenceStorageRow {
    ...
    private func fixture(_ label: String, migrated: Bool)
        throws -> (ElysiumStorageCoordinator, ElysiumRPGLocalPreferencesStorage) {
        let coordinator = try ElysiumStorageCoordinator.open(databaseURL: databaseURL(label))
        try coordinator.legacyCore().putWorldRow(
            ElysiumWorldStorageRow(id: "world", json: "{}", lastPlayed: 0))
    ...
    private var materializationRollbackStages: [ElysiumStorageRPGLocalFailureStage] {
        var stages: [ElysiumStorageRPGLocalFailureStage] = [.begin]
    ...
    private var worldDeleteRollbackStages: [ElysiumStorageRPGLocalFailureStage] {
        var stages: [ElysiumStorageRPGLocalFailureStage] = [.begin]
    ...
    func testWorldDeleteAfterCommitCutIsDurableAndRestartSafe() throws {
        ...
        let reopened = try ElysiumStorageCoordinator.open(databaseURL: databaseURL)
