...
final class PersistenceLockRankTests: XCTestCase {
    func testSupportedRankTransitionsRestoreExactPriorRank() {
        XCTAssertEqual(elysiumCurrentLockRank(), 0)
        let direct = withElysiumLockRank(.saveDB) { elysiumCurrentLockRank() }
        XCTAssertEqual(direct, ElysiumLockRank.saveDB.rawValue)
        XCTAssertEqual(elysiumCurrentLockRank(), 0)

        let migration = withElysiumLockRank(.migrationSource) {
            XCTAssertEqual(elysiumCurrentLockRank(), ElysiumLockRank.migrationSource.rawValue)
            return withElysiumLockRank(.saveDB) { elysiumCurrentLockRank() }
        }
        XCTAssertEqual(migration, ElysiumLockRank.saveDB.rawValue)

        let save = withElysiumLockRank(.saveQueue) {
            withElysiumLockRank(.saveDB) { elysiumCurrentLockRank() }
        }
        XCTAssertEqual(save, ElysiumLockRank.saveDB.rawValue)
        XCTAssertEqual(elysiumCurrentLockRank(), 0)
    }
    ...
    func testSaveQueueSelfEntryRunsInlineWithoutDeadlock() {
        ...
        XCTAssertEqual(game._testSaveQueueSelfEntry(), ElysiumLockRank.saveQueue.rawValue)
    }
    ...
    func testForwardReverseAndMarkerRollbackBoundariesUseCapturedIdentityAndSyncState() throws {
        let parent = FileManager.default.temporaryDirectory.appendingPathComponent(
            "elysium-rename-boundary-\(UUID().uuidString)", isDirectory: true)
    ...
    func testRankInversionsTrapInIsolatedSubprocesses() {
        if let scenario = ProcessInfo.processInfo.environment["ELYSIUM_RANK_TRAP_SCENARIO"] {
            switch scenario {
            case "12to11": withElysiumLockRank(.saveDB) { withElysiumLockRank(.saveQueue) {} }
            case "12to12": withElysiumLockRank(.saveDB) { withElysiumLockRank(.saveDB) {} }
            case "20to12": withElysiumLockRank(.publication) { withElysiumLockRank(.saveDB) {} }
        ...
        for scenario in ["12to11", "12to12", "20to12"] {
            ...
            child.arguments = ["ElysiumCoreTests.PersistenceLockRankTests/testRankInversionsTrapInIsolatedSubprocesses"]
            var environment = ProcessInfo.processInfo.environment
            environment["ELYSIUM_RANK_TRAP_SCENARIO"] = scenario
