...
final class LANV6ClientAuthorityCheckpointStorageTests: XCTestCase {
    ...
    private func url(_ label: String) -> URL {
        FileManager.default.temporaryDirectory.appendingPathComponent(
            "ElysiumClientCheckpoint-\(label)-\(UUID().uuidString).sqlite")
    }
    ...
    private func key(hidByte: UInt8 = 1, widByte: UInt8 = 2) throws
        ...
        return try ElysiumLANClientAuthorityStorageKey(
            hostInstallationID: hid, worldLANID: wid, lookupDigest: sha(input))
    }
    private func keyBytes(_ key: ElysiumLANClientAuthorityStorageKey) -> Data {
    ...
    private func credentialDigest(key: ElysiumLANClientAuthorityStorageKey,
    ...
    private func ownerDigest(key: ElysiumLANClientAuthorityStorageKey,
                             generation: UInt64, payload: Data) -> Data {
        var data = Data("Pebble/LANv6/client-owner/v1\0".utf8)
        data.append(keyBytes(key)); data.append(u64(generation)); data.append(payload)
        return sha(data)
    }
    private func aggregate(key: ElysiumLANClientAuthorityStorageKey, generation: UInt64,
    ...
    private func bootstrap(_ label: String) throws
        -> (URL, ElysiumStorageCoordinator, ElysiumClientAuthorityCheckpointV6Storage,
            ElysiumLANClientAuthorityStorageKey) {
    ...
    private func firstBindCandidate(key: ElysiumLANClientAuthorityStorageKey, token: Data,
        ...
        let aggregateDigest = aggregate(
            ...
            owner: (1, ownerDigest))
        let credential = try ElysiumLANClientCredentialStorageRow(
        ...
        let owner = try ElysiumLANClientOwnerCheckpointStorageRow(
            key: key, schemaVersion: 1, lastChangeGeneration: 1,
            payload: ownerPayload, payloadDigest: ownerDigest)
        return try ElysiumLANClientAuthorityCheckpointCandidate(
    ...
    func testCoreClientAuthorityAdapterIsAbsentUntilReviewedPhase25CodecsExist() throws {
        ...
        XCTAssertFalse(FileManager.default.fileExists(atPath: repository.appendingPathComponent(
            "Sources/ElysiumCore/Game/LANV6ClientCheckpointCodec.swift").path))
    ...
    func testPerScopePressurePrunesMinimumAcknowledgedRowInStableOrder() throws {
        ...
        let notice = try ElysiumLANClientNotificationStorageRow(
        ...
        let candidate = try ElysiumLANClientAuthorityCheckpointCandidate(
    ...
    func testStaleConcurrentCommitHasOneWinnerAndCommitFaultRollsBack() throws {
        ...
        let aggregate3 = aggregate(key: key, generation: 3, credentialDigest: digest3,
                                   owner: (1, first.snapshot.owner!.payloadDigest))
        let credential3 = try ElysiumLANClientCredentialStorageRow(
        ...
        let candidate3 = try ElysiumLANClientAuthorityCheckpointCandidate(
