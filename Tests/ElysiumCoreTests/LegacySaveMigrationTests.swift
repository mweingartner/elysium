...
final class LegacySaveMigrationTests: XCTestCase {
    private func makeParent(_ label: String) throws -> URL {
        let parent = FileManager.default.temporaryDirectory.appendingPathComponent(
            "elysium-legacy-migration-\(label)-\(UUID().uuidString)", isDirectory: true)
    ...
    private func makeRecoveryRestart(_ label: String) throws -> (URL, URL, URL) {
        ...
        let databaseURL = parent.appendingPathComponent("elysium.db")
    ...
    func testWorldStemMismatchFailsBeforeDatabasePublication() throws {
        ...
        XCTAssertThrowsError(try SaveDB.open(
            databaseURL: parent.appendingPathComponent("elysium.db"), migrateLegacy: true)) {
    ...
    func testPlayerAndAdvancementWrongDomainShapesAreSkippedIndependently() throws {
        ...
        let database = try SaveDB.open(databaseURL: parent.appendingPathComponent("elysium.db"),
                                       migrateLegacy: true)
    ...
    func testSecondOwnerAndSpecialFileNamespaceFailWithoutBlocking() throws {
        ...
        let start = Date()
        XCTAssertThrowsError(try SaveDB.open(
            databaseURL: fifoParent.appendingPathComponent("elysium.db"), migrateLegacy: true))
    ...
    func testCleanupMappingPreservesStageAndCloseFailureTombstonesLease() throws {
        ...
        XCTAssertThrowsError(try ElysiumStorageCoordinator.open(databaseURL: databaseURL)) {
            XCTAssertEqual($0 as? ElysiumStorageError, .duplicateOpen)
        }
    }
}
