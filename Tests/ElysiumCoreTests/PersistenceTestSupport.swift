...
enum PersistenceTestSupport {
    static func makeDatabase(owner: XCTestCase,
        ...
        let directory = FileManager.default.temporaryDirectory.appendingPathComponent(
            "elysium-persistence-tests-\(label)-\(UUID().uuidString)", isDirectory: true)
    ...
    static func makeGame(owner: XCTestCase, label: String = "game") -> GameCore {
        ...
        do {
            ...
            let database = try SaveDB.open(
                databaseURL: directory.appendingPathComponent("elysium.db"), migrateLegacy: false)
            ...
            XCTFail("unable to create unique Elysium test database")
            fatalError("unable to create unique Elysium test database")
        }
    }
}
