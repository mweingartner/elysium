...
final class SaveDBLifecycleTests: XCTestCase {
    private func makeURL(_ label: String) throws -> (directory: URL, database: URL) {
        let directory = FileManager.default.temporaryDirectory.appendingPathComponent(
            "elysium-save-lifecycle-\(label)-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        addTeardownBlock { try? FileManager.default.removeItem(at: directory) }
        return (directory, directory.appendingPathComponent("elysium.db"))
    }
    ...
    func testMigrationEnabledFactoryCreatesMissingParentBeforePreflight() throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "elysium-save-missing-parent-\(UUID().uuidString)", isDirectory: true)
        addTeardownBlock { try? FileManager.default.removeItem(at: root) }
        let databaseURL = root.appendingPathComponent("nested/elysium.db")
    ...
    func testConcurrentCloseReadAndWriteRemainBounded() throws {
        ...
        let database = try SaveDB.open(databaseURL: location.database, migrateLegacy: false)
        let queue = DispatchQueue(label: "elysium.lifecycle.test", attributes: .concurrent)
    ...
    func testOpenErrorDescriptionContainsOnlyClosedValues() {
        ...
        XCTAssertEqual(value.description,
                       "Elysium save open failed: migrationDecode/invalidSource")
    }
    ...
    func testStorageErrorMappingIsExhaustiveForOpenAndSchemaStages() {
        let cases: [(ElysiumStorageError, SaveDBOpenError.Result)] = [
    ...
    func testCompatibilityInitializerFatalErrorIsPathRedactedInSubprocess() throws {
        if ProcessInfo.processInfo.environment["ELYSIUM_COMPAT_INIT_CHILD"] == "1" {
            _ = SaveDB()
            return XCTFail("compatibility initializer unexpectedly returned")
        }
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "elysium-compat-redaction-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let blocker = root.appendingPathComponent("Library/Application Support/Elysium")
        ...
        let testName = "ElysiumCoreTests.SaveDBLifecycleTests/testCompatibilityInitializerFatalErrorIsPathRedactedInSubprocess"
        ...
        environment["ELYSIUM_COMPAT_INIT_CHILD"] = "1"
        environment["CFFIXED_USER_HOME"] = root.path
        let secret = "compat-secret-\(UUID().uuidString)"
        environment["ELYSIUM_SECRET_SENTINEL"] = secret
