...
final class LegacySaveMigrationAdversarialTests: XCTestCase {
    ...
    private func makeRoot(_ label: String) throws -> URL {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "elysium-migration-adversarial-\(label)-\(UUID().uuidString)", isDirectory: true)
        ...
        for delta in [-1, 0, 1] {
            ...
            if delta <= 0 {
                let database = try SaveDB.open(
                    databaseURL: root.appendingPathComponent("elysium.db"), migrateLegacy: true)
    ...
    func testFreshRegistrySubprocessAndTwoDatabaseAliasLeaseRace() throws {
        if let path = ProcessInfo.processInfo.environment["ELYSIUM_EMPTY_REGISTRY_CHILD"] {
            let database = try SaveDB.open(
        ...
        let testName = "ElysiumCoreTests.LegacySaveMigrationAdversarialTests/testFreshRegistrySubprocessAndTwoDatabaseAliasLeaseRace"
        ...
        environment["ELYSIUM_EMPTY_REGISTRY_CHILD"] = childRoot.appendingPathComponent("elysium.db").path
    ...
    func testImportBarrierRenameSyncReopenAndMarkerFailureRetryMatrix() throws {
        ...
        for cut in retryableSourceCuts {
            ...
            let databaseURL = root.appendingPathComponent("elysium.db")
    ...
    func testRealProcessRestartCutsBeforeAndAfterBarrierRenameAndParentSync() throws {
        if let rootPath = ProcessInfo.processInfo.environment["ELYSIUM_RESTART_CUT_ROOT"],
           let rawCut = ProcessInfo.processInfo.environment["ELYSIUM_RESTART_CUT"],
           let cut = LegacyMigrationTestCut(rawValue: rawCut) {
            ...
            _ = try SaveDB.open(databaseURL: URL(fileURLWithPath: rootPath)
                .appendingPathComponent("elysium.db"), migrateLegacy: true)
        ...
        for cut in cuts {
            ...
            let child = Process()
            child.executableURL = URL(fileURLWithPath: CommandLine.arguments[0])
            let testName = "ElysiumCoreTests.LegacySaveMigrationAdversarialTests/testRealProcessRestartCutsBeforeAndAfterBarrierRenameAndParentSync"
            if let bundle = CommandLine.arguments.last, bundle.hasSuffix(".xctest") {
                child.arguments = ["-XCTest", testName, bundle]
            } else {
                child.arguments = [testName]
            ...
            var environment = ProcessInfo.processInfo.environment
            environment["ELYSIUM_RESTART_CUT_ROOT"] = root.path
            environment["ELYSIUM_RESTART_CUT"] = cut.rawValue
            child.environment = environment
            let output = Pipe()
            let errors = Pipe()
