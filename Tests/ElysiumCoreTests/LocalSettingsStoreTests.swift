...
final class LocalSettingsStoreTests: XCTestCase {
    ...
    private func makeStore() throws -> LocalSettingsStore {
        let root = URL(fileURLWithPath: "/private/tmp", isDirectory: true)
            .appendingPathComponent("ElysiumLocalSettingsTests-\(UUID().uuidString)", isDirectory: true)
        roots.append(root)
    ...
    func testReadAndPersistRejectIntermediateAndParentDirectorySymlinks() throws {
        let base = URL(fileURLWithPath: "/private/tmp", isDirectory: true)
            .appendingPathComponent("ElysiumLocalSettingsTraversal-\(UUID().uuidString)", isDirectory: true)
        roots.append(base)
        let actual = base.appendingPathComponent("actual", isDirectory: true)
        let actualElysium = actual.appendingPathComponent("Elysium", isDirectory: true)
        try FileManager.default.createDirectory(at: actualElysium, withIntermediateDirectories: true)
        let original = Data("{\"fov\":91}".utf8)
        try original.write(to: actualElysium.appendingPathComponent("settings.json"))
        ...
        let intermediateStore = LocalSettingsStore(
            directoryURL: intermediateLink.appendingPathComponent("Elysium", isDirectory: true)
        )
        ...
        XCTAssertEqual(try Data(contentsOf: actualElysium.appendingPathComponent("settings.json")), original)

        let parentLink = base.appendingPathComponent("ElysiumLink", isDirectory: true)
        try FileManager.default.createSymbolicLink(at: parentLink, withDestinationURL: actualElysium)
