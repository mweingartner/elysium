...
final class SaveDBPlayerRowCASTests: XCTestCase {
    ...
    private func fixture(_ label: String) throws -> (SaveDB, URL) {
        let directory = FileManager.default.temporaryDirectory.appendingPathComponent(
            "ElysiumPlayerCAS-\(label)-\(UUID().uuidString)", isDirectory: true)
    ...
    func testDigestLengthExactBytesAndStoredJSONIdentity() throws {
        XCTAssertThrowsError(try ElysiumPlayerJSONRowDigest(
            data: Data(repeating: 0, count: 31))) {
                XCTAssertEqual($0 as? ElysiumStorageError, .invalidValue)
        }
        XCTAssertNoThrow(try ElysiumPlayerJSONRowDigest(data: Data(repeating: 0, count: 32)))
        XCTAssertThrowsError(try ElysiumPlayerJSONRowDigest(
    ...
    func testWorldAndJSONBoundariesStoreExactBytesAndRejectBeforeRank() throws {
        let directory = FileManager.default.temporaryDirectory.appendingPathComponent(
            "ElysiumPlayerCAS-boundaries-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        let coordinator = try ElysiumStorageCoordinator.open(
            databaseURL: directory.appendingPathComponent("elysium.db"))
        ...
        for (label, world) in [
            ...
            try storage.putWorldRow(try ElysiumWorldStorageRow(
                id: world, json: "{}", lastPlayed: 0))
            let candidate = try ElysiumPlayerJSONStorageRow(world: world, json: "{}")
        ...
        XCTAssertThrowsError(try ElysiumPlayerJSONStorageRow(
            world: String(repeating: "w", count: worldByteCap + 1), json: "{}")) {
                XCTAssertEqual($0 as? ElysiumStorageError, .limitExceeded)
        }
        ...
        for size in [playerJSONByteCap - 1, playerJSONByteCap] {
            ...
            let candidate = try ElysiumPlayerJSONStorageRow(world: world, json: json)
            XCTAssertEqual(try storage.compareAndSwapPlayerJSON(
    ...
    func testEveryStorageOperationFaultRestartsToExactOldState() throws {
        ...
        for operation in operations {
            for present in [false, true] {
                let directory = FileManager.default.temporaryDirectory.appendingPathComponent(
                    "ElysiumPlayerCASFault-\(operation.rawValue)-\(present)-\(UUID().uuidString)")
    ...
    func testCompatibilityPutPlayerPreservesRank11To12Ordering() throws {
        ...
        withElysiumLockRank(.saveQueue) {
            XCTAssertEqual(elysiumCurrentLockRank(), ElysiumLockRank.saveQueue.rawValue)
            database.putPlayer("world", ["revision": 1])
            XCTAssertEqual(elysiumCurrentLockRank(), ElysiumLockRank.saveQueue.rawValue)
        }
        XCTAssertEqual(elysiumCurrentLockRank(), 0)
