...
final class LANV6SchemaAuthorizerTests: XCTestCase {
    ...
    private func primaryStorageError(_ error: any Error) -> PebbleStorageError? {
        if let error = error as? PebbleStorageError { return error }
        if let error = error as? PebbleStorageTransactionFailure {
            return primaryStorageError(error.primary)
        }
        if let error = error as? PebbleStorageStatementFailure {
            return primaryStorageError(error.primary)
        }
        return nil
    }
    ...
    func testRuntimeScopesDenyWriteWideningCrossTableAndBootstrapReuse() throws {
        ...
        XCTAssertThrowsError(try coordinator._testReadScopeWriteProbe()) { error in
            XCTAssertEqual(error as? PebbleStorageError, .capabilityViolation)
        }
        XCTAssertThrowsError(try coordinator._testCrossTableMutationProbe()) { error in
            ...
            XCTAssertEqual(failure.primary as? PebbleStorageError, .capabilityViolation)
        }
    ...
    func testCanonicalDotDotSymlinkAndHardLinkAliasesCannotDoubleOpen() throws {
        ...
        let dotDot = url.deletingLastPathComponent()
            .appendingPathComponent("child/../pebble.db")
        XCTAssertThrowsError(try PebbleStorageCoordinator.open(databaseURL: dotDot)) { error in
        ...
        XCTAssertThrowsError(try PebbleStorageCoordinator.open(databaseURL: hardLink)) { error in
            XCTAssertEqual(error as? PebbleStorageError, .duplicateOpen)
        }
        ...
        XCTAssertThrowsError(try PebbleStorageCoordinator.open(databaseURL: symlink))
    }
        ...
        for url in [firstURL, secondURL] {
            ...
            DispatchQueue.global().async {
                ...
                do {
                    ...
                    try coordinator.close()
                } catch PebbleStorageError.duplicateOpen {
    ...
    func testHostileTriggerViewAndUnmanifestedIndexAreRejected() throws {
        ...
        for (label, sql) in fixtures {
            ...
            XCTAssertThrowsError(try PebbleStorageCoordinator.open(databaseURL: url), label) { error in
                XCTAssertEqual(error as? PebbleStorageError, .schemaMismatch)
            }
        }
    }
    ...
    func testSameColumnsWithMaliciousConstraintAndWrongWithoutRowIDAreRejected() throws {
        ...
        try runSQLite(constrainedURL, """
            ...
            """)
        XCTAssertThrowsError(try PebbleStorageCoordinator.open(databaseURL: constrainedURL)) { error in
            XCTAssertEqual(error as? PebbleStorageError, .schemaMismatch)
        }
        ...
        try runSQLite(rowIDURL, """
            ...
            """)
        XCTAssertThrowsError(try PebbleStorageCoordinator.open(databaseURL: rowIDURL)) { error in
            XCTAssertEqual(error as? PebbleStorageError, .schemaMismatch)
        }
    }
    ...
    func testLegacyTemplateSchemaUpgradesToExactManifest() throws {
        ...
        let row = try PebbleTemplateStorageRow(summary: summary, json: "{}", created: 0,
                                               format: 1, data: nil)
    ...
    func testWrongStorageClassAndOversizedTextRejectBeforeMaterialization() throws {
        ...
        var coordinator = try PebbleStorageCoordinator.open(databaseURL: wrongTypeURL)
        try coordinator.close()
        try runSQLite(wrongTypeURL, "INSERT INTO worlds(id,json,lastPlayed) VALUES('w',X'00',0.0)")
        coordinator = try PebbleStorageCoordinator.open(databaseURL: wrongTypeURL)
        XCTAssertThrowsError(try coordinator.legacyCore().getWorldRow(id: "w")) { error in
            XCTAssertEqual(error as? PebbleStorageError, .invalidStorageClass)
        }
        try coordinator.close()

        let oversizedURL = try databaseURL("oversized")
        coordinator = try PebbleStorageCoordinator.open(databaseURL: oversizedURL)
        try coordinator.close()
        ...
        coordinator = try PebbleStorageCoordinator.open(databaseURL: oversizedURL)
        XCTAssertThrowsError(try coordinator.legacyCore().getWorldRow(id: "w")) { error in
            XCTAssertEqual(error as? PebbleStorageError, .limitExceeded)
        }
        try coordinator.close()
    }
    ...
    func testWriteBoundsAtCapAndOneOver() throws {
        ...
        XCTAssertNoThrow(try PebblePlayerJSONStorageRow(world: "w", json: atCap))
        let oneOver = atCap + "x"
        XCTAssertThrowsError(try PebblePlayerJSONStorageRow(world: "w", json: oneOver)) { error in
            XCTAssertEqual(error as? PebbleStorageError, .limitExceeded)
        }
        XCTAssertThrowsError(try PebbleWorldStorageRow(id: "", json: "{}", lastPlayed: 0)) { error in
            XCTAssertEqual(error as? PebbleStorageError, .invalidValue)
        }
        XCTAssertThrowsError(try PebbleWorldStorageRow(id: "w", json: "{}", lastPlayed: .nan)) { error in
            XCTAssertEqual(error as? PebbleStorageError, .invalidValue)
        }
    }
    ...
    func testErrorsAndCompositeErrorsAreRedacted() throws {
        ...
        let transaction = PebbleStorageTransactionFailure(primary: primary,
                                                          rollback: .capabilityViolation,
                                                          terminal: nil)
        let statement = PebbleStorageStatementFailure(primary: primary, finalize: .statementLeak)
        ...
        XCTAssertFalse(String(describing: PebbleStorageError.invalidValue).contains(sentinel))
    }
    ...
    func testDeterministicPostOpenFailureClosesLocalHandleAndAllowsSafeReopen() throws {
        ...
        XCTAssertThrowsError(try PebbleStorageCoordinator._testOpen(
    ...
    func testFilesystemFailureIsClosedAndDoesNotRevealSentinelPath() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("PebbleStorage-SECRET-PATH-PAYLOAD-ID-\(UUID().uuidString)")
        ...
        let url = regularParent.appendingPathComponent("pebble.db")
        ...
        do { _ = try PebbleStorageCoordinator.open(databaseURL: url) }
        ...
        XCTAssertEqual(observed as? PebbleStorageError,
    ...
    func testEveryBootstrapDDLFailureRollsBackToExactPriorSnapshot() throws {
        for statementIndex in 0...15 {
            ...
            let restarted = try PebbleStorageCoordinator.open(databaseURL: url)
    ...
    func testLegacyCollectionScopesAllowFrozenReadsAndDenyAdjacentColumns() throws {
        let coordinator = try PebbleStorageCoordinator.open(databaseURL: databaseURL("legacy-scopes"))
        defer { try? coordinator.close() }
    ...
    func testLegacySingleReadsIgnoreEveryUnselectedCorruptColumn() throws {
        ...
        try storage.putWorldRow(PebbleWorldStorageRow(id: "w", json: "world-json", lastPlayed: 1))
        try storage.putPlayerJSON(PebblePlayerJSONStorageRow(world: "w", json: "player-json"))
        ...
        try storage.putAdvancementJSON(PebbleAdvancementStorageRow(world: "w", json: "adv-json"))
        let summary = try PebbleTemplateSummaryStorageRow(
            name: "template", sizeX: 1, sizeY: 2, sizeZ: 3,
            blockCount: 4, blockEntityCount: 5,
            dominantBlock: "stone", dominantDisplay: "Stone")
        _ = try storage.putTemplateRow(PebbleTemplateStorageRow(
        ...
        XCTAssertEqual(try storage.getLegacyTemplateContent(name: "template"),
                       PebbleLegacyTemplateContent(format: 2, data: Data([1, 2]),
                                                   json: "template-json"))
    ...
    func testLegacyTextMatrixRowLocalSkipRepairTruncateAndOptionalFallback() throws {
        ...
        XCTAssertEqual(try storage.getLegacyTemplateContent(name: "content"),
                       PebbleLegacyTemplateContent(format: nil, data: nil, json: nil))
        XCTAssertEqual(try storage.getLegacyTemplateContent(name: "legacy"),
                       PebbleLegacyTemplateContent(format: 1, data: nil,
                                                   json: "legacy-json"))
    ...
    func testLegacyLANOneQuerySkipsCorruptRowsSortsUTF8AndEnforces256Cap() throws {
        ...
        var storage = try coordinator.legacyCore()
        XCTAssertEqual(try storage.listLegacyLANPlayerJSON(world: "w"), [
            PebbleLegacyLANPlayerJSON(playerID: "a", json: "a"),
            PebbleLegacyLANPlayerJSON(playerID: "e\u{301}", json: "e"),
            PebbleLegacyLANPlayerJSON(playerID: "q", json: "q\u{FFFD}"),
            PebbleLegacyLANPlayerJSON(playerID: "z", json: "z"),
        ])
        ...
        coordinator = try PebbleStorageCoordinator.open(databaseURL: capURL)
        try coordinator.close()
    ...
    func testWrongClassZeroOneOutOfRangeAndDuplicateRootsRejectWithoutMutation() throws {
        ...
        for (index, mutation) in mutations.enumerated() {
            ...
            XCTAssertThrowsError(try PebbleStorageCoordinator.open(databaseURL: url), mutation.0)
    ...
    func testSchemaAuditAuthorizerDeniesAdjacentPragmasAndVirtualTables() throws {
        let coordinator = try PebbleStorageCoordinator.open(databaseURL: databaseURL("audit-adjacent"))
        defer { try? coordinator.close() }
        for probe in PebbleStorageSchemaAuditProbe.allCases {
    ...
    func testRestoredNotNullSchemaOverNullLegacyRowFailsClosedAtOpen() throws {
        ...
        XCTAssertThrowsError(try PebbleStorageCoordinator.open(databaseURL: url)) { error in
            XCTAssertEqual(self.primaryStorageError(error), .schemaIntegrity)
