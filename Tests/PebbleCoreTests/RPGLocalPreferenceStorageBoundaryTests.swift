...
final class RPGLocalPreferenceStorageBoundaryTests: XCTestCase {
    private func databaseURL(_ label: String) -> URL {
        FileManager.default.temporaryDirectory.appendingPathComponent(
            "PebbleRPGStorageBoundary-\(label)-\(UUID().uuidString).sqlite")
    }
    ...
    private func installWorld(_ id: String, core: PebbleLegacyCoreStorage) throws {
        try core.putWorldRow(PebbleWorldStorageRow(id: id, json: "{}", lastPlayed: 0))
    }
    ...
    func testPreferenceRowCount255_256_257AndAggregateCapMinusOneExact() throws {
        ...
        let facade = try coordinator.rpgLocalPreferences()
        var lastID = ""
        var lastRow: PebbleRPGLocalPreferenceStorageRow?
        for index in 0..<256 {
    ...
    func testLegacyMaterializationPrimitiveFailureRollbackAndRestartMatrix() throws {
        for operation: PebbleStorageOperationID in [
    ...
    func testMigrationRestartIdempotenceAndEqualNamedWorldIDsNeverCross() throws {
        ...
        var coordinator: PebbleStorageCoordinator? = try .open(databaseURL: url)
        let core = try coordinator!.legacyCore()
        try core.putWorldRow(PebbleWorldStorageRow(
