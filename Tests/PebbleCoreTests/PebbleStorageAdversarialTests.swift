...
final class PebbleStorageAdversarialTests: XCTestCase {
    private func databaseURL(_ label: String = UUID().uuidString) throws -> URL {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("PebbleStorageAdversarialTests-\(label)-\(UUID().uuidString)",
                                    isDirectory: true)
    ...
    private func schemaAggregateBytes(_ databaseURL: URL) throws -> Int {
        let value = try runSQLite(databaseURL, """
            ...
            FROM sqlite_master;
            """)
        guard let bytes = Int(value) else { throw PebbleStorageError.invalidValue }
        return bytes
    }
    ...
    private func makeExactSchemaAggregateFixture(targetBytes: Int, label: String) throws -> URL {
        ...
        guard let lastName = names.last else { throw PebbleStorageError.invalidValue }
        try runSQLite(url, """
            DROP INDEX "\(lastName)";
    ...
    func testExactAdvertisedChunkCapCanBePersisted() throws {
        let coordinator = try PebbleStorageCoordinator.open(databaseURL: databaseURL("chunk-cap"))
        defer { try? coordinator.close() }
        let storage = try coordinator.legacyCore()
        let key = try PebbleChunkStorageKey(world: "w", dimension: 0, chunkX: 0, chunkZ: 0)
        let row = try PebbleChunkStorageRow(key: key, data: Data(count: 67_108_864))
    ...
    func testEmptyChunkBatchStillHonorsClosedLifecycle() throws {
        let coordinator = try PebbleStorageCoordinator.open(databaseURL: databaseURL("closed-empty"))
        let storage = try coordinator.legacyCore()
        try coordinator.close()
    ...
    func testPostSQLiteOpenIdentityRaceDoesNotLeakAnUntrackedHandle() throws {
        ...
        for attempt in 0..<200 where !reproducedLeak {
            ...
            var coordinator: PebbleStorageCoordinator?
    ...
    func testCollectionByteAccountingIncludesAllUTF8OctetsAndEmbeddedNUL() throws {
        let coordinator = try PebbleStorageCoordinator.open(databaseURL: databaseURL("utf8-octets"))
        defer { try? coordinator.close() }
        let storage = try coordinator.legacyCore()
        ...
        let row = try PebbleWorldStorageRow(id: id, json: json, lastPlayed: 1)
    ...
    func testSchemaManifestAggregateAcceptsExactMiBAndRejectsOneByteOverBeforeInspection() throws {
        ...
        XCTAssertThrowsError(try PebbleStorageCoordinator.open(databaseURL: exactURL)) { error in
            XCTAssertEqual(error as? PebbleStorageError, .schemaMismatch,
        ...
        XCTAssertThrowsError(try PebbleStorageCoordinator.open(databaseURL: oneOverURL)) { error in
            XCTAssertEqual(error as? PebbleStorageError, .limitExceeded,
    ...
    func testWorstCaseChunkRowCapsAndExtremeCoordinatesPersistAtOnce() throws {
        ...
        let key = try PebbleChunkStorageKey(
            world: world, dimension: .max, chunkX: .min, chunkZ: .max)
        let below = try PebbleChunkStorageRow(key: key, data: Data(count: 67_108_863))
        XCTAssertEqual(try storage.putChunkBlobRows([below]), 1)
        XCTAssertEqual(try storage.getChunkBlob(key: key)?.count, 67_108_863)
        let exact = try PebbleChunkStorageRow(key: key, data: Data(count: 67_108_864))
        XCTAssertEqual(try storage.putChunkBlobRows([exact]), 1)
        XCTAssertEqual(try storage.getChunkBlob(key: key)?.count, 67_108_864)

        XCTAssertThrowsError(try PebbleChunkStorageRow(
            key: key, data: Data(count: 67_108_865))) { error in
            XCTAssertEqual(error as? PebbleStorageError, .limitExceeded)
        }
        XCTAssertThrowsError(try PebbleChunkStorageKey(
    ...
    func testTemplateShippedCapsSimultaneousAggregateAndNumericExtremesPersist() throws {
        ...
        let smallSummary = try PebbleTemplateSummaryStorageRow(
            name: "small", sizeX: 0, sizeY: 0, sizeZ: 0,
            blockCount: 0, blockEntityCount: 0,
            dominantBlock: "", dominantDisplay: "")
        XCTAssertThrowsError(try PebbleTemplateStorageRow(
    ...
    func testPrivateSQLiteWholeRecordCeilingIsExact() throws {
        let coordinator = try PebbleStorageCoordinator.open(databaseURL: databaseURL("row-ceiling"))
        defer { try? coordinator.close() }
    ...
    func testUniqueInteriorLeafAutoindexAndWithoutRowIDChildAliasesAreRejected() throws {
        ...
        for (index, value) in cases.enumerated() {
            ...
            XCTAssertThrowsError(try PebbleStorageCoordinator.open(databaseURL: url), value.label) {
                error in
    ...
    func testTerminalSQLiteSurfaceSourceGate() throws {
        ...
        let sourceURL = repository.appendingPathComponent("Sources/PebbleStorage/StorageEngine.swift")
    ...
    func testReleaseSurfaceVerifierIsExecutableFailClosedAndIntegrated() throws {
        let repository = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent().deletingLastPathComponent().deletingLastPathComponent()
        let verifierURL = repository.appendingPathComponent(
            "scripts/verify-pebble-storage-release-surface.sh")
        XCTAssertTrue(FileManager.default.isExecutableFile(atPath: verifierURL.path))
        ...
        for required in [
            ...
            "PebbleStorage.PebbleStorageCoordinator.open(databaseURL:",
            "_sqlite3_close_v2",
            "PebbleStorageTestDeadlineBoundary",
        ] {
            XCTAssertTrue(verifier.contains(required), "missing verifier invariant: \(required)")
        }
        XCTAssertFalse(verifier.contains("SKIP"))
    ...
    func testCloseV2UndefinedSymbolParserRejectsSpoofsAndAdjacentNames() throws {
        ...
        let verifier = try String(contentsOf: repository.appendingPathComponent(
            "scripts/verify-pebble-storage-release-surface.sh"), encoding: .utf8)
