...
final class RPGAccessibilitySourceTests: XCTestCase {
    ...
    func testAppKitBridgeCachesExactOriginStrongScreenAndNoCurrentTupleSubstitution() throws {
        let bridge = try source("Sources/Pebble/RPGAccessibilityM.swift")
        ...
        for retained in ["case .scalarRefresh(let retained)", "element.refreshScalars(",
                         "private func structuralKey", "PebbleRetainedStructureKey"] {
    ...
    func testGameViewGroupRolesCoordinatesAndNonactionablePressContract() throws {
        ...
        XCTAssertTrue(main.contains("Pebble menus and actions"))
    }
    ...
    func testFocusNotificationsAndAppearanceUseCommittedSemanticPath() throws {
        ...
        let screen = try source("Sources/Pebble/RPGScreensM.swift")
        ...
        let core = try source("Sources/PebbleCore/Game/RPGSemanticAccessibility.swift")
    ...
    func testAccessibilityFocusRequiresActiveKeyWindowAndRetainsOneEpochTarget() throws {
        let rpg = try source("Sources/Pebble/RPGAccessibilityM.swift")
        ...
        XCTAssertTrue(rpg.contains("func retainedFocusedElement() -> PebbleAccessibilityElement?"))
        ...
        let text = try source("Sources/Pebble/TextEntryAccessibilityM.swift")
        XCTAssertEqual(text.components(
            separatedBy: "guard NSApp.isActive, view?.window?.isKeyWindow == true,").count - 1, 2)
