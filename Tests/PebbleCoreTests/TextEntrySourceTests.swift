...
final class TextEntrySourceTests: XCTestCase {
    ...
    func testCanonicalKeyDownOwnsOrdinaryTextAndPreflightDoesNotFingerprintIt() throws {
        let router = try source("Sources/Pebble/AppInputRouterM.swift")
        ...
        XCTAssertTrue(router.contains(
            "private let exhaustionLatch = PebbleAppInputExhaustionLatch()"))
        ...
        XCTAssertFalse(router.contains("pebbleExhaustedInputDisposition("))
        XCTAssertTrue(router.contains("return source == .keyDown ? routeUnmappedScreenText(event) : false"))
        XCTAssertEqual(router.components(separatedBy: "PebbleTextEventIngressAdapter.route(").count - 1,
                       2)
    ...
    func testUIManagerOwnsCapabilityReadinessAndAllOrdinaryActivationPaths() throws {
        ...
        for token in ["private init(token: PebbleTextOwnerToken)",
                      "TextFocusAuthorization.mint(token)",
                      "PebbleTextFocusTransactionAdapter()",
        ...
        let screens = try source("Sources/Pebble/ScreensM.swift")
    ...
    func testAllShippingFieldsHaveStableUniqueIDsAndLimitsRemainAtCallSites() throws {
        let sources = try ["Sources/Pebble/MenusM.swift", "Sources/Pebble/LANLobbyScreen.swift",
                           "Sources/Pebble/ScreensM.swift"].map(source).joined(separator: "\n")
    ...
    func testPasteIsOwnerCapturedBoundedAndTextPrecedesTemplatePlacement() throws {
        ...
        XCTAssertTrue(paste.contains("PebbleTextPasteIngressAdapter.route"))
    ...
    func testTextAccessibilityCoexistsWithRPGAndHasNoValueMutationAPI() throws {
        let bridge = try source("Sources/Pebble/TextEntryAccessibilityM.swift")
        let main = try source("Sources/Pebble/main.swift")
    ...
    func testPackageTopologyHasNormalNonProductSupportTargetAndNoExecutableTestImport() throws {
        ...
        XCTAssertTrue(package.contains("name: \"PebbleTextInput\""))
        XCTAssertFalse(package.contains(".library(name: \"PebbleTextInput\""))
        XCTAssertFalse(package.contains("dependencies: [\"Pebble\"]"))
        let sources = try source("Sources/PebbleTextInput/PebbleTextInput.swift")
    ...
    func testNoSignSpecificLANWireOrReplicationSymbolsWereAdded() throws {
        let tree = try source("Sources/Pebble/ScreensM.swift")
    ...
    func testMachineGatesPinOneReleaseBuildBeforeAppKitAndDeploy() throws {
        ...
        let authority = try source("Sources/PebbleReleaseGate/ReleaseGate.swift")
    ...
    func testFailClosedPackagerAndZeroClipboardDriverAreDocumented() throws {
        ...
        let driver = try source("Tests/PebbleAppKitIntegration/Driver.swift")
    ...
    func testAppKitDriverUsesOneShotActionsPreRepairProofAndPerKeyBarriers() throws {
        ...
        let expectedIDs = [
            ...
            "finder.activate", "pebble.reactivate", "world-name.reactivation-focus",
    ...
    func testLaunchActivationIsProductOwnedOneShotAtTruthfulReveal() throws {
        ...
        for token in ["final class LaunchActivationAtRevealCoordinator",
                      "PebbleLaunchActivationState<ObjectIdentifier>",
                      "PebbleLaunchRevealToken<ObjectIdentifier>",
        ...
        let support = try source("Sources/PebbleAppSupport/PebbleAppSupport.swift")
        let consume = try XCTUnwrap(support.range(of: "public mutating func consume("))
        let input = try XCTUnwrap(support.range(
            of: "public enum PebbleAppInputEntrySource", range: consume.upperBound..<support.endIndex))
    ...
    func testConditionalDenialClickIsClosedOneShotAndNonshipping() throws {
        ...
        let package = try source("Package.swift")
        XCTAssertFalse(package.contains("Tests/PebbleAppKitIntegration"))
        let shipping = try ["Sources/Pebble/main.swift", "Sources/Pebble/AppInputRouterM.swift"]
    ...
    func testConditionalDenialV19OneShotBudgetBoundaryAndSourceOrdering() throws {
        ...
        for forbidden in ["+=", " max(", "reset", "rearm", "retry", "Sources/Pebble"] {
    ...
    func testLaunchPresentationV21BarrierBoundariesAndSourceOrdering() throws {
        ...
        for forbidden in ["+=", "restart", "rearm", "retry", "activate", "permission",
                          "clipboard", "Sources/Pebble", "CGEvent", "ConditionalDenial",
    ...
    func testConditionalDenialV15BoundAndExecutableDeadlineBoundaryMatrices() throws {
        ...
        let temporary = FileManager.default.temporaryDirectory.appendingPathComponent(
            "pebble-driver-self-test-\(UUID().uuidString)")
        ...
        compile.arguments = ["swiftc", root.appendingPathComponent(
            "Tests/PebbleAppKitIntegration/Driver.swift").path, "-o", temporary.path,
    ...
    func testInitialLaunchSettlementUsesTwoDistinctTurnSamplesAndLaterPhasesRemainFour() throws {
        ...
        let finderEnd = try XCTUnwrap(driver.range(
            of: "try activateOnce(\"pebble.reactivate\"",
    ...
    func testLaunchLifecycleBaselineAndPresentationBinderAreClosedAndDeterministic() throws {
        ...
        enum Event { case pebbleActivate, predecessorDeactivate, pebbleDeactivate,
        ...
        struct BaselineModel {
            ...
            mutating func event(_ value: Event, generation incoming: Int = 1) {
                ...
                switch value {
                case .pebbleActivate:
                    guard pair.insert("pebble.activate").inserted else { terminal = true; return }
                case .predecessorDeactivate:
                    guard pair.insert("predecessor.deactivate").inserted else {
                        terminal = true; return
                    }
                case .pebbleDeactivate, .otherActivate, .wrongDeactivate:
                    terminal = true; return
                }
        ...
        firstOrder.event(.pebbleActivate); firstOrder.event(.predecessorDeactivate)
        firstOrder.complete()
        ...
        secondOrder.event(.pebbleActivate)
        ...
        afterCompletion.event(.pebbleActivate); afterCompletion.event(.predecessorDeactivate)
        ...
        missing.complete(); missing.event(.pebbleActivate)
        XCTAssertFalse(missing.baseline(propertiesExact: true))
        missing.timeout(); XCTAssertTrue(missing.terminal)

        for event in [Event.pebbleDeactivate, .otherActivate, .wrongDeactivate] {
        ...
        for duplicate in [Event.pebbleActivate, .predecessorDeactivate] {
            var model = BaselineModel(); model.bind(.expectedLaunchPair); model.armAction()
            model.event(duplicate); model.event(duplicate)
            XCTAssertTrue(model.terminal)
        }
        var beforeBinding = BaselineModel(); beforeBinding.event(.pebbleActivate)
        XCTAssertTrue(beforeBinding.terminal)
        var afterPair = BaselineModel(); afterPair.bind(.expectedLaunchPair); afterPair.armAction()
        afterPair.event(.pebbleActivate); afterPair.event(.predecessorDeactivate)
        ...
        wrongGeneration.event(.pebbleActivate, generation: 2)
    ...
    func testWorkspaceActivationHandoffLedgerIsBoundOrderedAndRaceClosed() throws {
        ...
        for token in ["WorkspaceActivationHandoffLedger", "NSLock()", "generation: UInt64 = 1",
                      ...
                      "beginPebbleAction()", "awaitPebblePair(timeout: 2)",
        ...
        let pebble = try XCTUnwrap(driver.range(
            of: "activateOnce(\"pebble.reactivate\"",
            range: finderSettlement.upperBound..<driver.endIndex))
        let pebbleBoundary = try XCTUnwrap(driver.range(
            of: "handoffLedger.beginPebbleAction()", range: pebble.upperBound..<driver.endIndex))
        let pebbleAwait = try XCTUnwrap(driver.range(
            of: "handoffLedger.awaitPebblePair", range: pebbleBoundary.upperBound..<driver.endIndex))
        let settling = try XCTUnwrap(driver.range(
            of: "readyEpoch.settle(", range: pebbleAwait.upperBound..<driver.endIndex))
        ...
        XCTAssertLessThan(finderSettlement.lowerBound, pebble.lowerBound)
        XCTAssertLessThan(pebble.lowerBound, pebbleBoundary.lowerBound)
        XCTAssertLessThan(pebbleBoundary.lowerBound, pebbleAwait.lowerBound)
        XCTAssertLessThan(pebbleAwait.lowerBound, settling.lowerBound)

        func accepts(_ first: [String], _ second: [String]) -> Bool {
            Set(first) == Set(["finder.activate", "pebble.deactivate"]) && first.count == 2 &&
                Set(second) == Set(["pebble.activate", "finder.deactivate"]) && second.count == 2
        }
        XCTAssertTrue(accepts(["finder.activate", "pebble.deactivate"],
                              ["pebble.activate", "finder.deactivate"]))
        XCTAssertTrue(accepts(["pebble.deactivate", "finder.activate"],
                              ["finder.deactivate", "pebble.activate"]))
        XCTAssertFalse(accepts(["finder.activate"], ["pebble.activate", "finder.deactivate"]))
        XCTAssertFalse(accepts(["finder.activate", "finder.activate"],
                               ["pebble.activate", "finder.deactivate"]))
        XCTAssertFalse(accepts(["pebble.activate", "finder.deactivate"],
                               ["finder.activate", "pebble.deactivate"]))
        XCTAssertFalse(accepts(["unexpected.activate", "pebble.deactivate"],
                               ["pebble.activate", "finder.deactivate"]))
        ...
        struct FinderSample {
            ...
            var pebbleIdentity = true
            var pebbleInactive = true
            ...
            var authorityExact: Bool {
                finderIdentity && finderActive && finderFrontmost && pebbleIdentity &&
                    pebbleInactive && axWindowIdentity && axModeExact && axGeometryExact &&
        ...
        var pebbleActive = offscreen; pebbleActive.pebbleInactive = false
        ...
        for invalid in [finderInactive, finderNotFrontmost, pebbleActive, missing,
    ...
    func testInstalledSignoffReceiptIsExhaustiveAuthenticatedAndHookEnforced() throws {
        ...
        for token in ["isatty(STDIN_FILENO)", "isatty(STDOUT_FILENO)",
                      ...
                      "validateBoundPebbleProcess(pending)", "validateSealedInstalledEvidence(",
                      "pending.evidenceDigest", "pending.designerChallenge"] {
            XCTAssertTrue(designer.contains(token), token)
        }
        for forbidden in ["PebbleDesignerAttestationV2", "designer-attestation.json",
        ...
        for token in ["from: .prepared, to: .observedPendingDesigner",
                      ...
                      "PebbleDesignerAttestationV2", "designer-attestation.json",
