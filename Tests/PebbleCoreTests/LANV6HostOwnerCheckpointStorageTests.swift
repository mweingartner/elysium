...
final class LANV6HostOwnerCheckpointStorageTests: XCTestCase {
    private func url(_ label: String) -> URL {
        FileManager.default.temporaryDirectory.appendingPathComponent(
            "PebbleHostOwnerRows-\(label)-\(UUID().uuidString).sqlite")
    }
    ...
    func testDormantHostDDLManifestForeignKeysCapsAndNoWriterSourceContract() throws {
        ...
        let sourceURL = root.appendingPathComponent("Sources/PebbleStorage/StorageEngine.swift")
        ...
        XCTAssertFalse(source.contains("public final class PebbleHostOwner"))
