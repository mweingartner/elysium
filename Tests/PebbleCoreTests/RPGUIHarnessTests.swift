...
final class RPGUIHarnessTests: XCTestCase {
    private func bootstrap(_ selector: String,
        ...
        environment["PEBBLE_RPG_UI_CASE"] = selector
    ...
    func testNoCaseReturnsOrdinaryAndAllowlistIsExact() {
        ...
        XCTAssertEqual(RPGUIHarnessBootstrap.allowedPebbleKeys, Set([
            "PEBBLE_RPG_UI_CASE", "PEBBLE_RPG_UI_AUTHORITY", "PEBBLE_RPG_UI_VIEWPORT",
            "PEBBLE_RPG_UI_APPEARANCE", "PEBBLE_RPG_UI_SEMANTIC_SUMMARY", "PEBBLE_SHOT",
        ]))
        let shippingMutationKeys = [
            "PEBBLE_AUTOSTART", "PEBBLE_AUTOLOAD", "PEBBLE_NEW_WORLD", "PEBBLE_CMD",
            "PEBBLE_LAN_AUTOJOIN", "PEBBLE_LAN_PROBE", "PEBBLE_BOT",
            "PEBBLE_AI_ACTION_STUB", "PEBBLE_OPEN_SCREEN", "PEBBLE_RPG_AUTOCREATE",
            "PEBBLE_RPG_PATH", "PEBBLE_RPG_STARTER", "PEBBLE_RPG_SPELLS",
        ]
        for key in shippingMutationKeys {
            let result = RPGUIHarnessBootstrap.parseIfPresent([
                "PEBBLE_RPG_UI_CASE": "tab:warden:warden_guardian:character",
                key: "1",
            ])
            XCTAssertEqual(result, .rejected("RPG UI harness rejected: non-harness PEBBLE key"), key)
        }
    }

    func testBoundsAndDiagnosticsAreDeterministicAcrossInsertionOrders() {
        XCTAssertEqual(RPGUIHarnessBootstrap.parseIfPresent([
            "PEBBLE_RPG_UI_CASE": String(repeating: "x", count: 193),
        ]), .rejected("RPG UI harness rejected: environment value limit"))
        var tooMany = ["PEBBLE_RPG_UI_CASE": "tab:warden:warden_guardian:character"]
        ...
        var tooLarge = ["PEBBLE_RPG_UI_CASE": "tab:warden:warden_guardian:character"]
    ...
    func testShotAuthorityViewportAppearanceAndSummaryOptionsAreClosed() throws {
        let parsed = try bootstrap("tab:warden:warden_guardian:character", options: [
            "PEBBLE_RPG_UI_AUTHORITY": "acceptedCommit",
            "PEBBLE_RPG_UI_VIEWPORT": "700x420",
            "PEBBLE_RPG_UI_APPEARANCE": "highContrastReduceMotion",
            "PEBBLE_RPG_UI_SEMANTIC_SUMMARY": "1",
            "PEBBLE_SHOT": "rpg_fixture.png@600",
        ])
        ...
        for (key, value) in [
            ("PEBBLE_RPG_UI_AUTHORITY", "host"),
            ("PEBBLE_RPG_UI_VIEWPORT", "800x600"),
            ("PEBBLE_RPG_UI_APPEARANCE", "dark"),
            ("PEBBLE_RPG_UI_SEMANTIC_SUMMARY", "true"),
        ] {
            XCTAssertEqual(RPGUIHarnessBootstrap.parseIfPresent([
                "PEBBLE_RPG_UI_CASE": "tab:warden:warden_guardian:character", key: value,
            ]), .rejected(key == "PEBBLE_RPG_UI_SEMANTIC_SUMMARY"
            ...
            for rank in 1...3 {
                for state in ["purchased", "current", "nextLegal", "locked", "future"] {
                    ...
                    guard representable else {
                        XCTAssertEqual(RPGUIHarnessBootstrap.parseIfPresent([
                            "PEBBLE_RPG_UI_CASE": "skill:\(skill.id):\(rank):\(state)",
            ...
            for skill in RPG_SKILL_DEFINITIONS {
                for rank in 1...3 {
                    let built = try fixture(
                        ...
                        options: ["PEBBLE_RPG_UI_VIEWPORT": viewport.rawValue])
    ...
    func testStatusCompatibilityTargetsAndErrorCasesAreSemanticallyActive() throws {
        ...
        for invalid in [
            ...
            XCTAssertEqual(RPGUIHarnessBootstrap.parseIfPresent(["PEBBLE_RPG_UI_CASE": invalid]),
                           .rejected("RPG UI harness rejected: invalid case selector"), invalid)
        }
        XCTAssertEqual(RPGUIHarnessBootstrap.parseIfPresent([
            "PEBBLE_RPG_UI_CASE": "status:pending:saveSlots:slot4:authority",
        ...
        for invalidLifecycle in [
            ...
            var environment = ["PEBBLE_RPG_UI_CASE": invalidLifecycle]
            if invalidLifecycle.hasSuffix(":authority") {
                environment["PEBBLE_RPG_UI_AUTHORITY"] = "pending"
            }
        ...
        let lifecycleCases: [(RPGStatusKind, String, [String: String])] =
            RPGStatusKind.allCases.filter { $0 != .pending && $0 != .authorityExhausted }.map {
                ...
                (.pending, "authority", ["PEBBLE_RPG_UI_AUTHORITY": "pending"]),
                (.authorityExhausted, "authority",
                    ["PEBBLE_RPG_UI_AUTHORITY": "exhausted"]),
            ...
            for appearance in RPGUIHarnessAppearance.allCases {
                for viewport in RPGUIHarnessViewport.allCases {
                    let built = try fixture("tab:warden:warden_guardian:character", options: [
                        "PEBBLE_RPG_UI_AUTHORITY": authority.rawValue,
                        "PEBBLE_RPG_UI_APPEARANCE": appearance.rawValue,
                        "PEBBLE_RPG_UI_VIEWPORT": viewport.rawValue,
                    ])
    ...
    func testPureBootstrapAndFixtureDoNotWriteFreshSupportHome() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("pebble-rpg-harness-test-\(UUID().uuidString)", isDirectory: true)
