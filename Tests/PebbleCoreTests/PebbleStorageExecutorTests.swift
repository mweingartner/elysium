...
final class PebbleStorageExecutorTests: XCTestCase {
    private func databaseURL(_ name: String = UUID().uuidString) throws -> URL {
        ...
        return directory.appendingPathComponent("pebble.db")
    }
    ...
    private func runSQLite(_ databaseURL: URL, _ sql: String) throws -> String {
        ...
        guard process.terminationStatus == 0 else {
            ...
            throw PebbleStorageError.sqlite(primaryCode: process.terminationStatus,
    ...
    func testAllNamedLegacyFacadesRoundTripAndMutateAtomically() throws {
        ...
        let world = try PebbleWorldStorageRow(id: "w", json: "{}", lastPlayed: 1.5)
        ...
        let key = try PebbleChunkStorageKey(world: "w", dimension: 0, chunkX: -2, chunkZ: 3)
        let chunk = try PebbleChunkStorageRow(key: key, data: Data([0, 1, 2]))
        ...
        let player = try PebblePlayerJSONStorageRow(world: "w", json: "{\"p\":1}")
        try storage.putPlayerJSON(player)
        XCTAssertEqual(try storage.getPlayerJSON(world: "w"), player)

        let resume = try PebbleLANClientResumeStorageRow(hostWorld: "host", json: "{}", updated: 2)
        ...
        let lan = try PebbleLANPlayerStorageRow(world: "w", playerID: "p1", json: "{}", updated: 3)
    ...
    func testAtomicLegacyWorldImport() throws {
        ...
        defer { try? coordinator.close() }
        let storage = try coordinator.legacyCore()
        let world = try PebbleWorldStorageRow(id: "import", json: "{}", lastPlayed: 5)
        let player = try PebblePlayerJSONStorageRow(world: "import", json: "{}")
        let advancement = try PebbleAdvancementStorageRow(world: "import", json: "[]")
        let key = try PebbleChunkStorageKey(world: "import", dimension: 0, chunkX: 0, chunkZ: 0)
        let chunk = try PebbleChunkStorageRow(key: key, data: Data([1]))
        let value = try PebbleLegacyWorldImport(world: world, player: player,
    ...
    func testThreeHundredConcurrentWritesReadsAndKeysetPagesAreSerialized() throws {
        ...
        for index in 0..<300 {
            ...
            DispatchQueue.global().async {
                defer { group.leave() }
                do {
                    let row = try PebbleWorldStorageRow(id: String(format: "w-%03d", index),
    ...
    func testFailureInjectionRollsBackAndRestoresAutocommit() throws {
        ...
        for operation in operations {
            let coordinator = try PebbleStorageCoordinator.open(databaseURL: databaseURL(operation.rawValue))
            defer { try? coordinator.close() }
            let storage = try coordinator.legacyCore()
            try coordinator._testInject(operation)
            let row = try PebbleWorldStorageRow(id: "never", json: "{}", lastPlayed: 0)
    ...
    func testCaughtBindFailureCannotCommit() throws {
        ...
        defer { try? coordinator.close() }
        let storage = try coordinator.legacyCore()
        let row = try PebbleWorldStorageRow(id: "caught", json: "{}", lastPlayed: 0)
    ...
    func testBodyAndFinalizeFailurePreservesBothWithoutLeakingValues() throws {
        ...
        XCTAssertThrowsError(try coordinator._testBodyAndFinalizeFailure()) { error in
            guard let failure = error as? PebbleStorageStatementFailure else {
                return XCTFail("expected composite statement failure, got \(error)")
            }
            XCTAssertNotNil(failure.primary as? PebbleStorageTestBodyError)
    ...
    func testRollbackFailurePoisonsAndPreservesBothFailures() throws {
        ...
        let row = try PebbleWorldStorageRow(id: "poison", json: "{}", lastPlayed: 0)
        XCTAssertThrowsError(try storage.putWorldRow(row)) { error in
            guard let failure = error as? PebbleStorageTransactionFailure else {
    ...
    func testDeinitClosesHealthyHandleAndAllowsReopen() throws {
        ...
        do {
            let coordinator = try PebbleStorageCoordinator.open(databaseURL: url)
            let storage = try coordinator.legacyCore()
            try storage.putWorldRow(PebbleWorldStorageRow(id: "w", json: "{}", lastPlayed: 0))
        }
    ...
    func testCloseLinearizesAgainstConcurrentSubmittedWork() throws {
        ...
        for index in 0..<150 {
            ...
            DispatchQueue.global().async {
                defer { group.leave() }
                do {
                    try storage.putWorldRow(PebbleWorldStorageRow(
                        id: "race-\(index)", json: "{}", lastPlayed: Double(index)))
                } catch PebbleStorageError.closed {
        ...
        XCTAssertThrowsError(try coordinator.legacyCore()) { error in
            XCTAssertEqual(error as? PebbleStorageError, .closed)
        }
    ...
    func testEmptyChunkBatchUsesTransactionAdmissionAndCommitPath() throws {
        for operation in [PebbleStorageOperationID.beginImmediate, .commit] {
            let coordinator = try PebbleStorageCoordinator.open(databaseURL: databaseURL(operation.rawValue))
            defer { try? coordinator.close() }
            let storage = try coordinator.legacyCore()
            try coordinator._testInject(operation)
        ...
        let coordinator = try PebbleStorageCoordinator.open(databaseURL: databaseURL("empty-success"))
        defer { try? coordinator.close() }
    ...
    func testExtendedConstraintPreservesPrimaryMaskAndExtendedCode() throws {
        ...
        func sqliteFailure(in error: any Error) -> PebbleStorageError? {
            if let storage = error as? PebbleStorageError { return storage }
    ...
    func testDatabaseParentIdentityExactMismatchAliasAndClosedLifecycle() throws {
        ...
        XCTAssertThrowsError(try coordinator.verifyDatabaseParentIdentity(device: device, inode: inode)) {
            error in XCTAssertEqual(error as? PebbleStorageError, .closed)
        }
    }
    ...
    func testDatabaseParentReplacementPoisonsAndTombstonesPhysicalDatabase() throws {
        ...
        XCTAssertThrowsError(try coordinator.verifyDatabaseParentIdentity(device: device, inode: inode))
        XCTAssertThrowsError(try coordinator.legacyCore()) { error in
            XCTAssertEqual(error as? PebbleStorageError, .poisoned)
        }
        XCTAssertThrowsError(try PebbleStorageCoordinator.open(
            databaseURL: displaced.appendingPathComponent("pebble.db"))) { error in
            XCTAssertEqual(error as? PebbleStorageError, .duplicateOpen)
        }
    }

    func testReplaceCompleteImportRemovesStaleOptionalAndChunkStateButPreservesLANRows() throws {
        let coordinator = try PebbleStorageCoordinator.open(databaseURL: databaseURL("replace-import"))
        defer { try? coordinator.close() }
        let storage = try coordinator.legacyCore()
        let world = try PebbleWorldStorageRow(id: "w", json: "old", lastPlayed: 1)
        let oldKey = try PebbleChunkStorageKey(world: "w", dimension: 0, chunkX: 1, chunkZ: 1)
        let oldChunk = try PebbleChunkStorageRow(key: oldKey, data: Data([1]))
        _ = try storage.importLegacyWorld(PebbleLegacyWorldImport(
            world: world,
            player: PebblePlayerJSONStorageRow(world: "w", json: "old-player"),
            advancements: PebbleAdvancementStorageRow(world: "w", json: "old-adv"),
            chunks: [oldChunk]))
        let resume = try PebbleLANClientResumeStorageRow(hostWorld: "host", json: "resume", updated: 1)
        let peer = try PebbleLANPlayerStorageRow(
            world: "w", playerID: "peer", json: "peer-json", updated: 2)
        try storage.putLANClientResumeJSON(resume)
        try storage.putLANPlayerJSON(peer)

        let replacement = try PebbleLegacyWorldImport(
            world: PebbleWorldStorageRow(id: "w", json: "new", lastPlayed: 2),
    ...
    func testEveryReplaceCompleteImportFailureCutRestoresPriorAggregateAndLANRows() throws {
        struct Snapshot: Equatable {
            ...
            let advancement: PebbleAdvancementStorageRow?
            let chunkKeys: [PebbleChunkStorageKey]
            let chunkData: [Data]
            let resume: PebbleLANClientResumeStorageRow?
            let peers: [PebbleLANPlayerStorageRow]
        }

        func snapshot(_ storage: PebbleLegacyCoreStorage) throws -> Snapshot {
        ...
        for cut in PebbleStorageLegacyImportFailurePoint.allCases {
            let url = try databaseURL("import-cut-\(cut)")
            var coordinator = try PebbleStorageCoordinator.open(databaseURL: url)
            var storage = try coordinator.legacyCore()
            let oldChunks = try (0..<3).map { index in
                try PebbleChunkStorageRow(
                    key: PebbleChunkStorageKey(world: "w", dimension: 0,
                                               chunkX: Int32(index), chunkZ: 0),
                    data: Data([UInt8(index)]))
            }
            _ = try storage.importLegacyWorld(PebbleLegacyWorldImport(
                world: PebbleWorldStorageRow(id: "w", json: "old", lastPlayed: 1),
                player: PebblePlayerJSONStorageRow(world: "w", json: "old-player"),
                advancements: PebbleAdvancementStorageRow(world: "w", json: "old-adv"),
                chunks: oldChunks))
            try storage.putLANClientResumeJSON(PebbleLANClientResumeStorageRow(
                hostWorld: "host", json: "resume", updated: 1))
            try storage.putLANPlayerJSON(PebbleLANPlayerStorageRow(
                world: "w", playerID: "peer", json: "peer", updated: 2))
            let before = try snapshot(storage)
            let newChunks = try (10..<13).map { index in
                try PebbleChunkStorageRow(
                    key: PebbleChunkStorageKey(world: "w", dimension: 1,
                                               chunkX: Int32(index), chunkZ: -1),
                    data: Data([UInt8(index)]))
            }
            let replacement = try PebbleLegacyWorldImport(
                world: PebbleWorldStorageRow(id: "w", json: "new", lastPlayed: 2),
                player: PebblePlayerJSONStorageRow(world: "w", json: "new-player"),
                advancements: PebbleAdvancementStorageRow(world: "w", json: "new-adv"),
                chunks: newChunks)
            try coordinator._testSetLegacyImportFailure(cut)
            XCTAssertThrowsError(try storage.importLegacyWorld(replacement), "cut=\(cut)")
            try coordinator.close()

            coordinator = try PebbleStorageCoordinator.open(databaseURL: url)
    ...
    func testMigrationBarrierSuccessClosedFailuresRetryAndLifecycle() throws {
        let coordinator = try PebbleStorageCoordinator.open(databaseURL: databaseURL("barrier"))
        let storage = try coordinator.legacyCore()
        try storage.putWorldRow(PebbleWorldStorageRow(id: "w", json: "{}", lastPlayed: 0))
        XCTAssertNoThrow(try storage.prepareLegacyMigrationRename())
        XCTAssertEqual(try coordinator._testAuthorizationContract(), 0b1_1111)

        for point in [PebbleStorageBarrierFailurePoint.checkpointBusy,
            ...
            XCTAssertThrowsError(try storage.prepareLegacyMigrationRename(), "point=\(point)") {
                error in
                guard case let .sqlite(_, _, operation) = error as? PebbleStorageError else {
    ...
    func testMigrationBarrierRejectsLeakedStatementWithoutChangingScope() throws {
        ...
        try coordinator._testLeakRawStatementForClose()
        XCTAssertThrowsError(try storage.prepareLegacyMigrationRename()) { error in
            XCTAssertEqual(error as? PebbleStorageError, .statementLeak)
        }
    ...
    func testCloseInjectionOneProvesDisposalAndTwoRemainsPoisonedUntilDeinit() throws {
        ...
        let disposed = try PebbleStorageCoordinator.open(databaseURL: disposedURL)
        ...
        XCTAssertThrowsError(try PebbleStorageCoordinator.open(databaseURL: disposedURL))

        let poisonedURL = try databaseURL("close-poisoned")
        var poisoned: PebbleStorageCoordinator? = try PebbleStorageCoordinator.open(
        ...
        XCTAssertThrowsError(try PebbleStorageCoordinator.open(databaseURL: poisonedURL))
    }
    ...
    func testChunkPaginationRejectsEveryExternalCommitAndAcceptsReadOnlyProcess() throws {
        ...
        for mutation in mutations {
            ...
            if mutation.mustReject {
                ...
                XCTAssertEqual(capturedError as? PebbleStorageError, .schemaMismatch,
                               mutation.label)
            } else {
    ...
    func testStageEarlyDoubleResumeSingleUseTimeoutRetryAndCancellationRearm() throws {
        let coordinator = try PebbleStorageCoordinator.open(databaseURL: databaseURL("stages"))
        defer { try? coordinator.close() }
        let storage = try coordinator.legacyCore()
        ...
        XCTAssertEqual(capturedTimeout as? PebbleStorageTestStageError, .timeout)
        ...
        XCTAssertThrowsError(try wrongStage.waitUntilReached()) { error in
            XCTAssertEqual(error as? PebbleStorageTestStageError, .timeout)
        }
    ...
    func testDeadlineEqualityIsTerminalAndMatchingObservationDetaches() throws {
        let coordinator = try PebbleStorageCoordinator.open(databaseURL: databaseURL("deadline-equality"))
        defer { try? coordinator.close() }
        let storage = try coordinator.legacyCore()
        ...
        XCTAssertEqual(captured as? PebbleStorageTestStageError, .timeout)
    ...
    func testPostSyncIdentityFailureOutranksResumeAndStageTimeout() throws {
        for resume in [true, false] {
            ...
            XCTAssertNil(captured as? PebbleStorageTestStageError,
                         "identity failure must outrank stage timeout")
            XCTAssertThrowsError(try storage.listWorldRows()) { error in
                XCTAssertEqual(error as? PebbleStorageError, .poisoned)
            }
            XCTAssertThrowsError(try PebbleStorageCoordinator.open(
                databaseURL: displaced.appendingPathComponent("pebble.db")))
        }
    }
}
