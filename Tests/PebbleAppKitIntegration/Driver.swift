...
private final class WorkspaceActivationHandoffLedger {
    private enum Phase {
        case installing, prelaunch, preLaunchActionArmed, actionPending,
             ...
             boundNavigation, finderAction, finderCommitted, pebbleAction, committed, ready,
             failed, closed
    }
    ...
    private enum Category: String {
        case provisionalTarget, pebble, expectedPredecessor, expectedFinder, unexpectedOther
    }
    ...
    private var pebble: ExpectedApplication?
    ...
    private var launchPebbleActivated = false
    ...
    private var pebbleDeactivated = false
    private var pebbleActivated = false
    ...
    func bindTargetAndReconcile(
        ...
        for event in events {
            ...
            if targetEventExact, !launchPebbleActivated {
                launchPebbleActivated = true
        ...
        } else if launchPebbleActivated && predecessorDeactivated {
    ...
    func beginLaunchBaselineSample() throws -> UInt64? {
        ...
        if phase == .targetBoundAwaitingIngressDrain {
            ...
            if launchPebbleActivated && predecessorDeactivated {
                launchPairCompletionSequence = UInt64(events.count)
        ...
        if let pebble {
            guard !pebble.object.isTerminated,
                  pebble.object.processIdentifier == pebble.pid,
                  pebble.object.bundleURL.map({ canonicalURL($0.path) }) == pebble.bundle,
                  pebble.object.executableURL.map({ canonicalURL($0.path) }) ==
                    pebble.executable else {
    ...
    func beginLaunchSettlementSample() throws -> LaunchSettlementLease? {
        ...
        case .zeroEventSurface:
            guard events.isEmpty, observedIngress == 0,
                  !launchPebbleActivated, !predecessorDeactivated,
    ...
    func armSyntheticDenialClick(lease: LaunchSettlementLease) throws {
        ...
        guard !syntheticDenialActionArmed, launchPath == .expectedLaunchPair,
              ...
              !launchPebbleActivated, !predecessorDeactivated else {
    ...
    func beginPebbleAction() throws {
        try transition(from: .finderCommitted, to: .pebbleAction, label: "pebble_action_boundary")
    }
    ...
    func awaitPebblePair(timeout: TimeInterval) throws {
    ...
    private func record(_ notification: Notification, kind: Kind, generation: UInt64) {
        ...
        case .targetBoundAwaitingIngressDrain, .launchPairAwaitingEvents:
            ...
            if classified.category == .pebble, kind == .activate, !launchPebbleActivated {
                launchPebbleActivated = true
        ...
        case .launchSettlement:
            ...
            guard classified.identityExact,
                  (classified.category == .pebble && kind == .activate ||
            ...
            if classified.category == .pebble { launchPebbleActivated = true }
        ...
        case .finderAction:
            ...
            } else if classified.category == .pebble, kind == .deactivate, !pebbleDeactivated {
                pebbleDeactivated = true
            } else { failLocked("finder_pair_duplicate_or_order"); return }
            if finderActivated && pebbleDeactivated { phase = .finderCommitted }
        case .pebbleAction:
            guard events.count < 32 else { failLocked("event_capacity"); return }
            events.append(event)
            guard classified.identityExact, classified.category != .unexpectedOther else {
            ...
            if classified.category == .pebble, kind == .activate, !pebbleActivated {
                pebbleActivated = true
            ...
            if pebbleActivated && finderDeactivated { phase = .committed }
    ...
    private func classify(_ candidate: NSRunningApplication)
        ...
        if let pebble, candidate.processIdentifier == pebble.pid {
            return (.pebble, candidate.isEqual(pebble.object) &&
                candidate.bundleIdentifier == staticTarget.bundleIdentifier &&
                candidate.bundleURL.map({ canonicalURL($0.path) }) == pebble.bundle &&
                candidate.executableURL.map({ canonicalURL($0.path) }) == pebble.executable)
        }
        if pebble == nil,
           candidate.bundleIdentifier == staticTarget.bundleIdentifier,
    ...
    private func samePhase(_ lhs: Phase, _ rhs: Phase) -> Bool {
        ...
        case (.installing, .installing), (.prelaunch, .prelaunch),
             ...
             (.finderCommitted, .finderCommitted), (.pebbleAction, .pebbleAction),
    ...
    private func diagnosticsLocked() -> String {
        ...
        return "handoff terminal=\(terminalReason ?? "none") " +
            ...
            "pebble_activate_seen=\(launchPebbleActivated) " +
            ...
            "completion_bound=\(pebble != nil) buffer_reconciled=\(pebble != nil) " +
            "finder_bound=\(finder != nil) " +
            "finder_pair_committed=\(finderActivated && pebbleDeactivated) " +
            "pebble_pair_committed=\(pebbleActivated && finderDeactivated) " +
...
private struct FinderSurfaceObservation {
    ...
    let pebbleIdentity: Bool
    let pebbleInactive: Bool
    ...
    var authorityExact: Bool {
        finderIdentity && finderActive && finderFrontmost && pebbleIdentity && pebbleInactive &&
    ...
    var aggregate: String {
        [
            ...
            ("finder_frontmost", finderFrontmost), ("pebble_identity", pebbleIdentity),
            ("pebble_inactive", pebbleInactive), ("ax_window_identity", axWindowIdentity),
...
do {
    ...
    guard manifest["bundle_id"] == "com.briangao.pebble" else {
    ...
    let staticArtifactAuthority = StaticArtifactAuthority(
        bundle: bundleURL, bundleIdentifier: "com.briangao.pebble",
    ...
    guard !app.isTerminated, app.processIdentifier > 0,
          app.bundleIdentifier == "com.briangao.pebble",
            ...
            if let window = try? currentWindow() {
                let matches = descendants(window).filter {
                    axString($0, kAXRoleAttribute as CFString) == kAXGroupRole &&
                        axString($0, kAXDescriptionAttribute as CFString) == "Pebble menus and actions"
                }
    ...
    try finderSurfaceSettlement.settle(
        ...
        sample: {
            ...
            return FinderSurfaceObservation(
                ...
                pebbleIdentity: !app.isTerminated &&
                    app.processIdentifier == boundAppIdentity.pid &&
    ...
    try activateOnce("pebble.reactivate", app, ledger: actionLedger, finalCheck: {
        guard app.processIdentifier == cleanup.expectedPID,
              app.bundleURL.map({ canonicalURL($0.path) }) == bundleURL,
              app.executableURL.map({ canonicalURL($0.path) }) == stagedExecutable else {
            throw GateError.failed("pebble reactivation adjacent identity")
        }
        try finderSurfaceSettlement.requireSettled()
        try handoffLedger.requireFinderSurfaceFinal()
        try handoffLedger.beginPebbleAction()
    })
    try handoffLedger.awaitPebblePair(timeout: 2)
