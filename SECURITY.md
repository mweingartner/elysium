...
Pebble is a local macOS game with player-started LAN multiplayer session support. Local-network multiplayer is deliberately scoped to Bonjour discovery, Direct Connect, bounded handshakes, peer status, LAN chat, and host-authoritative replication batches. Clients can request actions with typed intents, but they cannot send raw save data, arbitrary commands, or authoritative world state; the remaining full remote-player gameplay work is tracked in [LAN_MULTIPLAYER_PLAN.md](LAN_MULTIPLAYER_PLAN.md).
