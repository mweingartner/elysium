...
- **Full survival loop** — mining with tool tiers, hunger/saturation, XP, sleeping, fall damage, drowning, fire, status effects, death messages, durable bed/respawn-anchor spawn points, respawn/keep-inventory game rules.
...
| `PEBBLE_LAN_PROBE=host-rig` or `client-door` | test hook used by `scripts/live-lan-test.sh` to prove two-Mac LAN door use and block-entity snapshot sharing |
