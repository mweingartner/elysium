...
- **Selectable visual styles and resource packs** — Java Edition-style resource packs are read through Elysium's bounded archive and metadata loaders. **Options... → Video → Resource Packs...** selects either the default [Faithful 64x](https://faithfulpack.net/faithful64x) Release 12 style or the locally packaged **KUBIKOS Cubic World** alternative. KUBIKOS is generated only on a machine with the separately licensed source package and is not included in this public source checkout. The reviewed Ore Borders 64x and Static Lanterns add-ons remain independently selectable, off by default, and apply only to Faithful.
...
nothing. Ordinary tools, including pickaxes, use the active visual-style/resource-pack pixels with
...
The standalone scan also verifies every managed visual-style archive. On a checkout that does not
already contain the Git-ignored KUBIKOS build artifact, first run
`bash scripts/prepare-kubikos-theme.sh` with its separately licensed local source package available.
...
packaging/                 App metadata, branding, icons, public texture assets, and local KUBIKOS build metadata
...
- **Faithful style:** the bundled [Faithful 64x](https://faithfulpack.net/faithful64x) texture set is the work of the Faithful team and its contributors. The reviewed [Ore Borders 64x](https://faithfulpack.net/addons/OreBorders64x) and [Static Lanterns](https://faithfulpack.net/addons/ClearerLanterns) add-ons remain separate, optional Faithful-only layers. They are distributed under the separate [Faithful License](packaging/FAITHFUL-LICENSE.txt), with archive hashes and exact add-on attribution in [FAITHFUL-ADDONS-CREDITS.txt](packaging/FAITHFUL-ADDONS-CREDITS.txt), and are not covered by Elysium's MIT license.
- **KUBIKOS Cubic World style:** the optional alternate style is a deterministic, Elysium-specific raster conversion of the licensed [KUBIKOS - Cube World](https://www.fab.com/listings/7bbf72cf-03bd-458f-9a74-1ab85bd1f2e5) art by ANIMMAL Game Assets. It applies KUBIKOS material detail while retaining Elysium semantic masks, alpha, silhouettes, and UI layout. The converter uses only declared coherent material faces or named repeatable shader/emission surfaces; it never treats a full Unity UV unwrap as a tile texture. The source listing disallows AI use; this conversion uses no AI generation or training. Neither the licensed Unity package nor the generated KUBIKOS archive is stored in this public repository: [scripts/prepare-kubikos-theme.sh](scripts/prepare-kubikos-theme.sh) verifies a locally licensed input and creates a Git-ignored packaging artifact for a local Elysium.app build. Do not commit, publish, extract, resell, or reuse that archive independently; see [KUBIKOS-ATTRIBUTION.txt](packaging/KUBIKOS-ATTRIBUTION.txt) and the applicable [Fab Standard License](https://www.fab.com/eula).
