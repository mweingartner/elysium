...
// Elysium packages the intentionally headless adapter without Arnis' GUI and Luanti
// feature consumers. Keep that supported feature slice warning-free without muting
// warnings in Arnis' normal GUI build.
...
fn run_cli() {
    ...
    let world_format = if args.elysium {
        world_editor::WorldFormat::ElysiumExchange
    ...
    let (generation_path, level_name) = if args.elysium {
        let output = args
            .path
            .clone()
            .expect("validated Elysium output directory");
        (output, None)
    } else if args.bedrock {
    ...
    if args.overture && args.include_buildings && !skip_objects {
    ...
    let (raw_data, overture_elements, mut ground) = std::thread::scope(|s| {
        let overture_handle = s.spawn(|| {
            ...
            let elements = if args.overture && args.include_buildings && !skip_objects {
    ...
    if !args.include_buildings {
        parsed_elements.retain(|element| {
    ...
    } else if args.overture && args.include_buildings && !skip_objects {
    ...
    match data_processing::generate_world_with_options(
        ...
        Ok(_) => {
            ...
            if world_format == world_editor::WorldFormat::JavaAnvil {
