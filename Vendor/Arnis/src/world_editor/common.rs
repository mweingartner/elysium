...
impl SectionToModify {
    #[inline]
    pub(crate) fn exchange_block_at(&self, index: usize) -> Block {
        self.storage.get(index)
    }

    #[inline]
    pub(crate) fn exchange_properties_at(&self, index: usize) -> Option<&Value> {
        self.properties.get(&index).map(AsRef::as_ref)
    }
}
