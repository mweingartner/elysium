...
- **`GameCore`** — the orchestrator. Fixed 20 Hz tick (50 ms), with chunk generation on a concurrent queue (capped in-flight), meshing on its own queue, and saves on a serial queue. Chunks are generated off-main and only published to the world on the main thread (`adoptChunk`), which is the threading contract that keeps the engine lock-free. Autosave every 60 s; unloads batch their writes into one SQLite transaction per second. Each loaded host chunk remembers the entity ids a reload of it would reproduce (its record's entities, or its worldgen spawns); autosave rewrites chunks with edited blocks plus chunks whose entity membership changed (a birth, death, arrival, or departure — never-edited chunks stay cheap entity-only records), while unload, the terminal save on exit, and an explicit checked save capture every loaded chunk in every dimension that holds entities or already has a record, in fixed dimension/chunk order. Chunk loads prefer a record still buffered for the batch write or handed to the save queue but uncommitted over the database row, so a chunk revisited within a second of unloading keeps its entities.
- **Spawn placement** — natural spawning keeps `canSpawnAt` (biome, light, ground, and fluid rules; water-filled flora such as underwater seagrass counts as water). Direct spawn paths — raid waves, pillager patrols, and the AI companion's summon — use the shared, RNG-free `spawnPlacementIsValid`: aquatic creatures (squid, fish, dolphins, guardians, axolotls, tadpoles, and the aquatic prehistoric roster) go only into water, land creatures never into water, lava, or water-filled flora, amphibians (drowned, turtles, frogs) and objects (boats, minecarts, end crystals) only avoid lava and obstruction, and prehistoric bodies also need whole-body clearance. A raid or patrol member sampled over water moves to the nearest valid surface column within four blocks or is skipped; a wave with no valid member is re-rolled up to three times before the raid moves on, and a raid that never places a single raider ends without victory or Hero of the Village. Generated ordinary structure occupants are not materialized when a land creature's body would stand in a fluid.
...
- Revision (predation limits, applied in place to existing v2/v3 saves): `PrehistoricPredationPolicy` lets a land predator hunt only herd prey no longer than 1.5x its own authoring length, and a predator that kills starts no new hunt for 12000 of its own ticks (half a day). The satiation deadline is session-only (a resumed predator is hungry again), so there is no save or LAN field, and neither rule draws RNG. The prehistoric dawn census applies each category cap per eligible player's 128-block neighbourhood, so herds parked in the non-ticking outer loaded ring no longer hold every refill slot and one LAN player's full region cannot consume another's vacancies; normal worlds keep the whole-loaded-world census.
- Revision (herd encounters, September 26, 2026): herds used to fight to the death while a predator's generic panic reflex cancelled its own attack on every hit, so rallies killed most predators. `PrehistoricHerdEncounterPolicy` makes v2+ land predators hold their ground while healthy (no generic panic, retaliation that ignores herds while wary or fed, family knockback resistance), retreat when at 40% health, outnumbered by three defenders or already fed, and stay wary of herds afterward; rallies continue only while the predator struck the herd within 200 ticks and stays within 18 blocks; land creatures recover out of combat. All state is session-only and RNG-free. The attrition harness now ends a day with 6/6 predators and 7-8/12 herbivores (docs/PREHISTORIC_WORLDS.md).
